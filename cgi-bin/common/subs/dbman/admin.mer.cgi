	...
	if ($in{'process'}) {
					...
					if ($is_valid_bill) {
						if ($amount_available > 0 and $amount_bill <= $amount_available and $amount_remaining > 0 and $amount_bill <= $amount_remaining) {
							...
							if ($sth->rows() == 1){
								...
								my ($sth) = &Do_SQL("/* $bill_type,$id_vendors,$vendor_category,$currency_exchange */ SELECT IF(ID_purchaseorders > 0 AND sl_bills_pos.Amount > 0,ID_purchaseorders,0) AS ID_po, IF(ID_bills_expenses > 0 AND sl_bills_expenses.Amount > 0,ID_bills_expenses,0) AS ID_expense, $amount / sl_bills.Amount FROM sl_bills  LEFT JOIN sl_bills_pos USING(ID_bills) LEFT JOIN sl_bills_expenses USING(ID_bills) WHERE sl_bills.ID_bills = '".$add_id_creddep."' HAVING ID_po > 0 OR ID_expense > 0 ORDER BY sl_bills_pos.Amount, sl_bills_expenses.Amount;");
								my $rows = $sth->rows();

								while(my($id_po, $id_expenses, $paid_pct) = $sth->fetchrow()) {
									...
									if($id_po) {
										...
										&Do_SQL("INSERT INTO sl_bills_notes SET ID_bills = '$add_id_creddep', Notes='Deposit Applied\nFrom Bill: $id_bills\nOriginal Amount: $this_amount\nCurrency Exchange: currency_exchange\nTotal Paid: ".round($this_amount * $exchange_rate,3)." ',Type='Low', Date = CURDATE(), Time = CURTIME(), ID_admin_users='$usr{'id_admin_users'}' ");
								...
								my $query_to_paid = "UPDATE sl_movements SET ID_journalentries = 0,tablerelated = 'sl_bills', ID_tablerelated = '$id_bills' WHERE $modquery AND (ID_tablerelated = 0 OR ID_tablerelated IS NULL) AND TIMESTAMPDIFF(SECOND,CONCAT(Date,' ',Time) , NOW()) BETWEEN 0 AND 5 AND ID_admin_users = '$usr{'id_admin_users'}';";
		...
		if ($va{'bills_total_amount'} > 0 and $in{'id_ordersttia'} and $va{'bills_total_amount'} <= $va{'bills_total_amount_due'}) {
			...
			if (!$in{'id_banks'} or $in{'id_banks'} eq ""){
				...
				my ($sth) = &Do_SQL("SELECT ID_accounts FROM sl_accounts INNER JOIN sl_banks ON sl_accounts.`Description` = `ABA-ACH` WHERE ID_banks = '$in{'id_banks'}' AND ID_accategories = '1';");
								...
								for my $i (0..$#arr_amount) {
									if ($arr_amount[$i] > 0){
										if ($arr_amount[$i] <= $arr_amount_due[$i]){
											...
											my ($sth) = &Do_SQL("SELECT ID_vendors,Category,IF(ID_purchaseorders > 0 AND sl_bills_pos.Amount > 0,ID_purchaseorders,0) AS ID_po, IF(ID_bills_expenses > 0 AND sl_bills_expenses.Amount > 0,ID_bills_expenses,0) AS ID_expense, $amount / sl_bills.Amount FROM sl_bills INNER JOIN sl_vendors USING(ID_vendors) LEFT JOIN sl_bills_pos USING(ID_bills) LEFT JOIN sl_bills_expenses USING(ID_bills) WHERE sl_bills.ID_bills = '".$id_bills."' HAVING ID_po > 0 OR ID_expense > 0 ORDER BY sl_bills_pos.Amount, sl_bills_expenses.Amount;");
											my $rows = $sth->rows();

											while(my($id_vendors, $vendor_category, $id_po, $id_expenses, $paid_pct) = $sth->fetchrow()) {
												...
												if($id_po) {
													...
													my @params = ($id_po,$id_bills,$ida_banks,$this_amount,$in{'currency_exchange'});
											...
											my $modquery = $str_pos ne '' ? "ID_tableused IN ($str_pos) AND tableused = 'sl_purchaseorders'" : "ID_tableused IN ($str_bills) AND tableused = 'sl_bills'";
											my $query_to_paid = "UPDATE sl_movements SET EffDate = '".&filter_values($in{'bankdate'})."', ID_journalentries = 0 WHERE $modquery AND TIMESTAMPDIFF(SECOND,CONCAT(Date,' ',Time) , NOW()) BETWEEN 0 AND 10 AND ID_admin_users = '$usr{'id_admin_users'}';";
											&Do_SQL($query_to_paid);
	...
	if($flag and !$in{'process'}) {
		...
		if ($va{'matches'} > 0 and $in{'id_vendors'}){
			...
			my ($sth) = &Do_SQL("SELECT *
				...
				AND Amount > 0 
				$query ORDER BY DueDate LIMIT $first,$usr{'pref_maxh'}; ");
		...
		my ($sth) = &Do_SQL("SELECT COUNT(*) FROM sl_bills WHERE 1
		AND (if(Type='Deposit',Status ='Paid',Status IN('Processed','Partly Paid')))
