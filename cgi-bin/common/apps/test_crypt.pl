...
#################################################################
	...
	print "<h4>DIREKSYS $cfg{'app_title'} (e$in{'e'}) - TESTING CRYPT $process</h5>";
...
print qq|<span style="color:red;">Orig-->$text</span><br />|;

my $result = &LeoEncrypt($text);
print qq|<span style="color:red;">Enc-->$result</span><br />|;

my $devolv = &LeoDecrypt($result);
print qq|<span style="color:red;">Dec-->$devolv</span><br />|;
...
my ($sth) = &Do_SQL("
	SELECT 
		...
		, PmtField3enc
		, ID_orders_payments
	FROM sl_orders_payments WHERE Type='Credit-Card' 
	AND PmtField3 IS NOT NULL
	AND TRIM(PmtField3) != ''
	-- LIMIT 1000;
");
print qq|<table cellpadding="5" border="1">|;
print qq|<tr><td>Original</td><td>Enc</td><td>Des</td><td>Dif</td><td>SQL</td></tr>|;
while ($rec = $sth->fetchrow_hashref()){
	my $enc = &LeoEncrypt($rec->{'PmtField3'});
	
	my $sql = "UPDATE sl_orders_payments SET PmtField3enc='$enc' WHERE ID_orders_payments='$rec->{'ID_orders_payments'}' LIMIT 1;";
	&Do_SQL($sql) if ($in{'process'} eq 'commit');
	
	my $dec = &LeoDecrypt($rec->{'PmtField3enc'});
	my $diff = ($rec->{'PmtField3'} ne $dec)? 'Diference':'';
	
	print qq|<tr><td>|.$rec->{'PmtField3'}.qq|</td><td>|.$rec->{'PmtField3enc'}.qq|</td><td>$dec</td><td>$diff</td><td>$sql</td></tr>|;
}
print qq|</table>|;
			...
			if (-e  $inbound_file_path . $file_name) {
				...
				LINE: while (<$this_file>) {
					...
					if($this_order_data[31] eq 'CC'){
						print qq|<h4>Procesando Pedido de tipo <span style="color:blue;">Tarjeta de Credito</span></h4>|;
					}
					if($this_order_data[31] eq 'COD'){
						print qq|<h4>Procesando Pedido de tipo <span style="color:blue;">COD</span></h4>|;
					}
					if($this_order_data[31] eq 'DR'){
						print qq|<h4>Procesando Pedido de tipo <span style="color:blue;">Deposito Referenciado</span></h4>|;
					}
					...
					if ($test){
						...
						print qq|<span style="color:red;">CP no corresponde al Estado: $test</span><br>|;
						$err++;
					...
					if ($this_order_data[0]){
						...
						if ($repeated_order){
							...
							print qq|<span style="color:red;">El pedido $this_order_data[0] ya existe en el sistema con el ID: $repeated_order</span><br>|;
							$err++;
							$str_error .= $req_fields_cod[$_] ."|";
						}
					}
					...
					if($this_order_data[31] eq 'CC'){
						...
						$this_order_data[35] = int($this_order_data[35]);
						...
						if (!$this_order_data[36] or length($this_order_data[36]) != 4){
							...
							print qq|<span style="color:red;">Formato de columna 36 incorrecto MMYY: $this_order_data[36]</span><br>|;
							$err++;
							...
							# print $month."<---".$this_order_data[36];
						}
						if (!$this_order_data[34] or length($this_order_data[34]) < 15){
							...
							print qq|<span style="color:red;">Formato de columna 34 incorrecto 9999 9999 9999 9999: $this_order_data[34]</span><br>|;
							$err++;
						}
						if (!$this_order_data[35] or length($this_order_data[35]) < 3){
							$log_email .= qq|<span style="color:red;">Formato de columna 35 incorrecto int(3-4): $this_order_data[35]</span><br>|;
							print qq|<span style="color:red;">Formato de columna 35 incorrecto int(3-4): $this_order_data[35]</span><br>|;
							$err++;
						}
						if (!$this_order_data[38] or (length($this_order_data[38]) != 1 and length($this_order_data[38]) != 3 and length($this_order_data[38]) !=6 and length($this_order_data[38]) != 9 and length($this_order_data[38]) != 12) ){
							$log_email .= qq|<span style="color:red;">Formato de columna 38 incorrecto (1,3,6,9,12): $this_order_data[38]</span><br>|;
							print qq|<span style="color:red;">Formato de columna 38 incorrecto (1,3,6,9,12): $this_order_data[38]</span><br>|;
							$err++;
						}
					}


					my $tr1 = qq|<table border="0" cellpadding="8" cellspacing="0" width="100%" style="font-size:10px; border: 1px solid #222222;">|;
					$tr1 .= qq|	<tr>|;
					my $tr2 = qq|	<tr>|;
					...
					for(0..$#this_order_data){
						...
						$tr1 .= qq|<td style="font-size:10px; border: 1px solid #222222;" nowrap>[$_] $req_fields_cc[$_]</td>|;
						$tr2 .= qq|<td style="font-size:10px; border: 1px solid #222222;" nowrap>$column $valid</td>|;
					}
					...
					if($err){
						...
						print qq|\n\n<h3 style="color:red;">Se detectaron $err error(es), corrijalos para continuar.</h3><br>\n\n|;
						# return;
					}
					...
					# print "\n\n$sql<br>\n\n" if ($in{'debug'} ==1);
					print_query($sql) if ($in{'debug'} == 1);
					&add_order_notes_by_type($data{'ID_orders'},"Este pedido fue cargado al sistema, mediante un procedimiento masivo. Archivo $file_name","Low") if ($in{'process'} eq 'commit');
