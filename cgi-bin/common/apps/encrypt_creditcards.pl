...
#################################################################
#################################################################
#	Function: execute_encrypt_creditcards
#		Función para extraer y enmascarar el número y cvn de la tarjeta de los plogs
#   		
#
...
#   	See Also:
#
sub execute_encrypt_creditcards{
	...
	print '<h4 style="border-bottom: 3px double gray;">DIREKSYS '.$cfg{'app_title'}.' e('.$in{'e'}.') :: Extracci&oacute;n del N&uacute;mero de Tarjeta de los PLogs F:'.$in{'fid'}.' <span style="position: relative; float: right;">Process: '.$process.'</span></h4>';
	...
	#Id user
	my $id_user = ( $in{'u'} ) ? $in{'u'} : '11'; 
	
	# First row process
	my $fisrt_id_op = ( $in{'fid'} && $in{'fid'} > 0) ? $in{'fid'} : 0;
	# my $fisrt_row_process = $in{'first'};
	# if( !$fisrt_row_process ){
	# 	print qq|<span style="color:red;">ID_orders de inicio es requerido</span>|;
	# 	return;
	# }
	...
	while ( $refMain = $sthMain->fetchrow_hashref() ) {
		...
		# 		$data_plogs =~ s/Cvv2Val=\d{3}/Cvv2Val=xxxx/g;
		...
		#&Do_SQL("UPDATE sl_orders_payments SET PmtField3='".$cc_mask."', PmtField4='xxxx', PmtField5='xxxx' WHERE ID_orders_payments=".$id_payment.";");
		##------------------------------------------------
