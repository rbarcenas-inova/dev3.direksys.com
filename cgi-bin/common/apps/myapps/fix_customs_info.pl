...
#################################################################
#################################################################
#	Function: execute_fix_customs_info
#
#
...
sub execute_fix_customs_info{
	...
	print '<h4 style="border-bottom: 3px double gray;">DIREKSYS '.$cfg{'app_title'}.' e('.$in{'e'}.') :: Fix Customs Info <span style="position: relative; float: right;">Process: '.$process.'</span></h4>';
 ...
 #    $id_products_trans = 108872 if ($in{'e'} == 11);
	...
	my ($sthMain) = &Do_SQL("SELECT ID_products_trans, ID_products
							FROM sl_skus_trans
							WHERE sl_skus_trans.Date >= '2015-08-01' 
								AND sl_skus_trans.tbl_name='sl_orders' 
								AND sl_skus_trans.ID_customs_info IS NULL 
							GROUP BY sl_skus_trans.ID_trs;");

	&Do_SQL("BEGIN;");	
	while ( $refMain = $sthMain->fetchrow_hashref() ) {
		my $id_products = $refMain->{'ID_products'};
		my $id_trans = $refMain->{'ID_products_trans'};
		...
		###----------------------------------------------------
		my $sql = "SELECT COUNT(*) Cont, ID_customs_info
					FROM sl_warehouses_location
					WHERE ID_products=$id_products 
						AND ID_customs_info IS NOT NULL
					GROUP BY ID_customs_info
					ORDER BY Cont DESC
					LIMIT 1;";
		my $sthCus = &Do_SQL($sql);
		my $id_customs_info = $sthCus->fetchrow();

		if( $id_customs_info and $id_customs_info > 0 ){

			### Actualiza el left_quantity_total del SKU
			my $sql = "UPDATE sl_skus_trans SET ID_customs_info = $id_customs_info WHERE ID_products_trans=$id_trans;";
			&Do_SQL($sql);
			$logs .= $sql."\n";
			$logs .= "ID editado :: $id_trans\n";
			print $sql."<br>";
			print "ID editado :: $id_trans<br>";
