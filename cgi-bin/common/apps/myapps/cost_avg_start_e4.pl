...
#############################################################################
	...
	my @cols = ('sku','upc','cost_avg','cost_adj');
			...
			LINE: while (<$this_file>) {
				...
				if($this_data{'sku'} ne '' and $this_data{'cost_avg'} > 0){
					...
					if( int($id_last_trans) > 0 ){
						...
						&Do_SQL("INSERT INTO `sl_skus_trans` (`ID_products`, `ID_warehouses`, `Type`, `Type_trans`, `tbl_name`, `Quantity`, `Cost_Avg`, `Cost`, Cost_Adj, `Date`, `Time`, `ID_admin_users`) 
