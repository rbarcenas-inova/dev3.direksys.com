...
sub execute_fix_groups_perms{
	...
	my ($sthMain) = &Do_SQL("SELECT ID_admin_reports
							 FROM admin_reports;");
	my $i = 0;
	&Do_SQL("BEGIN;");	
	while ( $refMain = $sthMain->fetchrow_hashref() ) {
		++$i;
		my $id_rpt = $refMain->{'ID_admin_reports'};
		
		
		print '<tr>';
		print '	<td style="font-size: 14pt; font-weight: bold;">ID_admin_reports: '.$id_rpt.'</td>';
		print '</tr>';
		print '<tr><td>';

		$logs .= "----------------------------------------------------------------------------\n";
		$logs .= "--- Inicia el proceso del ID_admin_reports: $id_rpt\n";
		$logs .= "----------------------------------------------------------------------------\n";		
		## Se agrega la regla para restringir el reporte actual en el mod. admin
		my $qry_upd_rpt = "REPLACE INTO admin_groups_perms SET ID_admin_groups=6, application='admin', command='repmans$id_rpt';\n";
		$logs .= $qry_upd_rpt;
		&Do_SQL($qry_upd_rpt);
		print '<br />'.$qry_upd_rpt;
		## Se agrega la regla para restringir el reporte actual en el mod. crm
		my $qry_upd_rpt = "REPLACE INTO admin_groups_perms SET ID_admin_groups=6, application='crm', command='repmans$id_rpt';\n";
		$logs .= $qry_upd_rpt;
		&Do_SQL($qry_upd_rpt);
		print '<br />'.$qry_upd_rpt;
		## Se agrega la regla para restringir el reporte actual en el mod. wms
		my $qry_upd_rpt = "REPLACE INTO admin_groups_perms SET ID_admin_groups=6, application='wms', command='repmans$id_rpt';\n";
		$logs .= $qry_upd_rpt;
		&Do_SQL($qry_upd_rpt);
		print '<br />'.$qry_upd_rpt;
	...
	if( $in{'process'} ne 'commit' ){
		...
		my $file_name = 'fix_groups_perms_log_'.$date.'.txt';
