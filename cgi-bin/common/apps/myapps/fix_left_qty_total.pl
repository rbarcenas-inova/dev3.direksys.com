...
sub execute_fix_left_qty_total{
 ...
 #    $id_products_trans = 108872 if ($in{'e'} == 11);
	...
	my ($sthMain) = &Do_SQL("Select ID_products From sl_skus_trans Where /*Date>='2015-01-30' And*/ ID_products>0 Group by ID_products;");
	...
	while ( $refMain = $sthMain->fetchrow_hashref() ) {
		...
		if( $left_qty_total and $left_qty_total > 0 ){
			
			my $sth = Do_SQL("SELECT ID_products_trans FROM sl_skus_trans WHERE ID_products=$id_products ORDER BY ID_products_trans DESC LIMIT 1;");
			my $ids = $sth->fetchrow();

			### Actualiza el left_quantity_total del SKU
			my $sql = "UPDATE sl_skus_trans SET left_quantity_total = $left_qty_total /*, Cost_Avg=$cost*/ WHERE ID_products=$id_products ORDER BY ID_products_trans DESC LIMIT 1;";
			&Do_SQL($sql);
			$logs .= $sql."\n";
