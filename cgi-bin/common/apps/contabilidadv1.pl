...
#################################################################
	...
	/* AND cu_invoices_lines.ID_orders = '9356869' */
	...
	while ( my $rec = $sth->fetchrow_hashref()){
		...
		VALUES (NULL, '$cta_banco', '$rec->{'invoice_total'}', '', '$rec->{'doc_date'}', 'sl_orders', '$rec->{'ID_orders'}', 'cu_invoices', '$rec->{'ID_invoices'}', 'Anticipo Clientes', 'Debit', 0, 0, 'Active', CURDATE(), CURTIME(), 1);";
		&print_query($sql);
		&Do_SQL($sql) if ($in{'process'} eq 'commit');
		
		$sql = "INSERT INTO sl_movements (ID_movements, ID_accounts, Amount, Reference, EffDate, tableused, ID_tableused, tablerelated, ID_tablerelated, Category, Credebit, ID_segments, ID_journalentries, Status, Date, Time, ID_admin_users) 
		VALUES (NULL, '$cta_anticipo', '$rec->{'invoice_total'}', '', '$rec->{'doc_date'}', 'sl_orders', '$rec->{'ID_orders'}', 'cu_invoices', '$rec->{'ID_invoices'}', 'Anticipo Clientes', 'Credit', 0, 0, 'Active', CURDATE(), CURTIME(), 1);";
		&print_query($sql);
		&Do_SQL($sql) if ($in{'process'} eq 'commit');

		$sql = "INSERT INTO sl_movements (ID_movements, ID_accounts, Amount, Reference, EffDate, tableused, ID_tableused, tablerelated, ID_tablerelated, Category, Credebit, ID_segments, ID_journalentries, Status, Date, Time, ID_admin_users) 
		VALUES (NULL, '$cta_anticipo', '$rec->{'invoice_total'}', '', '$rec->{'doc_date'}', 'sl_orders', '$rec->{'ID_orders'}', 'cu_invoices', '$rec->{'ID_invoices'}', 'Ventas', 'Debit', 0, 0, 'Active', CURDATE(), CURTIME(), 1);";
		&print_query($sql);
		&Do_SQL($sql) if ($in{'process'} eq 'commit');
	...
	)sl_creditmemos_products ON sl_creditmemos_products.ID_creditmemos=cu_invoices_lines.ID_creditmemos
	LEFT JOIN (
		SELECT 
	...
	)sl_creditmemos_payments ON sl_creditmemos_payments.ID_creditmemos=cu_invoices_lines.ID_creditmemos
	WHERE cu_invoices.Status='Certified'
	AND cu_invoices.invoice_type='egreso'
	AND cu_invoices_lines.ID_creditmemos > 0
	AND DATE(cu_invoices.doc_date) >= '$in{'date_from'}'
	AND DATE(cu_invoices.doc_date) < '$in{'date_to'}'
	/* AND sl_creditmemos_payments.ID_orders='9356869' */
	GROUP BY cu_invoices_lines.ID_invoices
	ORDER BY cu_invoices_lines.ID_invoices";
	&print_query($sql);
	my ($sth) = &Do_SQL($sql);
	...
	while ( my $rec = $sth->fetchrow_hashref()){
		...
		# Se modifican Status a sl_movements
		$sql = "UPDATE sl_movements SET Status='Direksys_Active' WHERE tableused='sl_orders' AND ID_tableused='$rec->{'ID_orders'}' AND Status='Active' AND DATE < CURDATE() AND DATE >= '2014-01-01'";
		&print_query($sql);
		&Do_SQL($sql) if ($in{'process'} eq 'commit');
		
		$sql = "UPDATE sl_movements SET Status='Direksys_Pending' WHERE tableused='sl_orders' AND ID_tableused='$rec->{'ID_orders'}' AND Status='Pending' AND DATE < CURDATE() AND DATE >= '2014-01-01'";
		&print_query($sql);
		&Do_SQL($sql) if ($in{'process'} eq 'commit');
		...
		## Se insertan Mov Contables
		$sql = "INSERT INTO sl_movements (ID_movements, ID_accounts, Amount, Reference, EffDate, tableused, ID_tableused, tablerelated, ID_tablerelated, Category, Credebit, ID_segments, ID_journalentries, Status, Date, Time, ID_admin_users) 
		VALUES (NULL, '$cta_ventas', '$rec->{'SalePrice'}', '', '$rec->{'doc_date'}', 'sl_orders', '$rec->{'ID_orders'}', 'cu_invoices', '$rec->{'ID_invoices'}', 'Devoluciones', 'Debit', 0, 0, 'Active', CURDATE(), CURTIME(), 1);";
		&print_query($sql);
		&Do_SQL($sql) if ($in{'process'} eq 'commit');

		$sql = "INSERT INTO sl_movements (ID_movements, ID_accounts, Amount, Reference, EffDate, tableused, ID_tableused, tablerelated, ID_tablerelated, Category, Credebit, ID_segments, ID_journalentries, Status, Date, Time, ID_admin_users) 
		VALUES (NULL, '$cta_envio', '$rec->{'Shipping'}', '', '$rec->{'doc_date'}', 'sl_orders', '$rec->{'ID_orders'}', 'cu_invoices', '$rec->{'ID_invoices'}', 'Devoluciones', 'Debit', 0, 0, 'Active', CURDATE(), CURTIME(), 1);";
		&print_query($sql);
		&Do_SQL($sql) if ($rec->{'Shipping'} > 0 and $in{'process'} eq 'commit');

		$sql = "INSERT INTO sl_movements (ID_movements, ID_accounts, Amount, Reference, EffDate, tableused, ID_tableused, tablerelated, ID_tablerelated, Category, Credebit, ID_segments, ID_journalentries, Status, Date, Time, ID_admin_users) 
		VALUES (NULL, '$cta_iva', '$rec->{'total_taxes_transfered'}', '', '$rec->{'doc_date'}', 'sl_orders', '$rec->{'ID_orders'}', 'cu_invoices', '$rec->{'ID_invoices'}', 'Devoluciones', 'Debit', 0, 0, 'Active', CURDATE(), CURTIME(), 1);";
		&print_query($sql);
		&Do_SQL($sql) if ($rec->{'total_taxes_transfered'} > 0 and $in{'process'} eq 'commit');

		$sql = "INSERT INTO sl_movements (ID_movements, ID_accounts, Amount, Reference, EffDate, tableused, ID_tableused, tablerelated, ID_tablerelated, Category, Credebit, ID_segments, ID_journalentries, Status, Date, Time, ID_admin_users) 
		VALUES (NULL, '$cta_costo', '$rec->{'Cost'}', '', '$rec->{'doc_date'}', 'sl_orders', '$rec->{'ID_orders'}', 'cu_invoices', '$rec->{'ID_invoices'}', 'Devoluciones', 'Credit', 0, 0, 'Active', CURDATE(), CURTIME(), 1);";
		&print_query($sql);
		&Do_SQL($sql) if ($in{'process'} eq 'commit');

		$sql = "INSERT INTO sl_movements (ID_movements, ID_accounts, Amount, Reference, EffDate, tableused, ID_tableused, tablerelated, ID_tablerelated, Category, Credebit, ID_segments, ID_journalentries, Status, Date, Time, ID_admin_users) 
		VALUES (NULL, '$cta_inventario', '$rec->{'Cost'}', '', '$rec->{'doc_date'}', 'sl_orders', '$rec->{'ID_orders'}', 'cu_invoices', '$rec->{'ID_invoices'}', 'Devoluciones', 'Debit', 0, 0, 'Active', CURDATE(), CURTIME(), 1);";
		&print_query($sql);
		&Do_SQL($sql) if ($in{'process'} eq 'commit');
		
		$sql = "INSERT INTO sl_movements (ID_movements, ID_accounts, Amount, Reference, EffDate, tableused, ID_tableused, tablerelated, ID_tablerelated, Category, Credebit, ID_segments, ID_journalentries, Status, Date, Time, ID_admin_users) 
		VALUES (NULL, '$cta_reembolso', '$rec->{'invoice_total'}', '', '$rec->{'doc_date'}', 'sl_orders', '$rec->{'ID_orders'}', 'cu_invoices', '$rec->{'ID_invoices'}', 'Devoluciones', 'Credit', 0, 0, 'Active', CURDATE(), CURTIME(), 1);";
		&print_query($sql);
		&Do_SQL($sql) if ($in{'process'} eq 'commit');
		
		$sql = "INSERT INTO sl_movements (ID_movements, ID_accounts, Amount, Reference, EffDate, tableused, ID_tableused, tablerelated, ID_tablerelated, Category, Credebit, ID_segments, ID_journalentries, Status, Date, Time, ID_admin_users) 
		VALUES (NULL, '$cta_banco', '$rec->{'invoice_total'}', '', '$rec->{'doc_date'}', 'sl_orders', '$rec->{'ID_orders'}', 'cu_invoices', '$rec->{'ID_invoices'}', 'Reembolsos', 'Credit', 0, 0, 'Active', CURDATE(), CURTIME(), 1);";
		&print_query($sql);
		&Do_SQL($sql) if ($in{'process'} eq 'commit');

		$sql = "INSERT INTO sl_movements (ID_movements, ID_accounts, Amount, Reference, EffDate, tableused, ID_tableused, tablerelated, ID_tablerelated, Category, Credebit, ID_segments, ID_journalentries, Status, Date, Time, ID_admin_users) 
		VALUES (NULL, '$cta_reembolso', '$rec->{'invoice_total'}', '', '$rec->{'doc_date'}', 'sl_orders', '$rec->{'ID_orders'}', 'cu_invoices', '$rec->{'ID_invoices'}', 'Reembolsos', 'Debit', 0, 0, 'Active', CURDATE(), CURTIME(), 1);";
		&print_query($sql);
		&Do_SQL($sql) if ($in{'process'} eq 'commit');
