...
local ($home_dir) = '/home/www/domains/sldev.shoplatinotv_new.com/cgi-bin/common/apps/'; 
...
sub fixcustorder{
	...
	$sth = &Do_SQL("SELECT ID_orders,ID_customers,TRIM(concat( if( isnull(Address1 ) , '', Address1 ) , ' ', if( isnull( Address2 ) , '', Address2 ) , ' ', if( isnull( Address3 ) , '', Address3 ) , ' ', Zip, ' ', state )),shp_name,
			...
			FROM sl_orders WHERE ID_orders >= 132000 ORDER BY ID_orders");
...
<Order>
  ...
  <Customer_Info>
    ...
    <Customer_Address>$customer_new_add</Customer_Address>
		...
		if($addn eq $customer_new_add){
			$file = "same_customers.xml";
			$flag=0;
			$ok++;
		}else{
			$sth2 = &Do_SQL("SELECT  ID_customers,CONCAT(FirstName,' ',LastName1,' ',LastName2) FROM sl_customers WHERE 
					TRIM(concat( if( isnull( Address1 ) , '', Address1 ) , ' ', if( isnull( Address2 ) , '', Address2 ) , ' ', if( isnull( Address3 ) , '', Address3 ) , ' ', Zip, ' ', state )) = '$addn';");
			...
			if($sth2->rows() == 0){
				...
				$bug++;
				$maindata .= "
  <Error>SELECT  ID_customers,CONCAT(FirstName,' ',LastName1,' ',LastName2) FROM sl_customers WHERE TRIM(concat( if( isnull( Address1 ) , '', Address1 ) , ' ', if( isnull( Address2 ) , '', Address2 ) , ' ', if( isnull( Address3 ) , '', Address3 ) , ' ', Zip, ' ', state )) = '$addn';</Error>";
			}else{
