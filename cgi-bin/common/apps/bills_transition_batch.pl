...
#############################################################################
	...
	my @cols = ('id_vendors','type','currency','date','amount','invoice','memo','currency_exchange','tax','id_segments','acc_debit','deductible');
	...
	FILE: foreach my $file_name (sort @files) {
		...
		if (-e  $inbound_file_path . $file_name) {
			...
			LINE: while (<$this_file>) {
				...
				$this_data{'deposit_id_accounts'} = 1118; # 105 PROV SER NAL
				...
				$this_data{'bill_id_accounts'} = $this_data{'acc_debit'};#600; # Cuenta contable del debito
				$this_data{'bill_id_tax_accounts'} = 1114; # Cuenta contable del debito
				...
				if($this_data{'id_vendors'} > 0 and $this_data{'type'} and $this_data{'date'} and $this_data{'amount'}){
					...
					if($this_data{'id_bills'}) {
						...
						}elsif($this_data{'type'} eq 'Credit'){
							###################################################################
							###################################################################
							###################################################################
							###################################################################
					...
					++$bills_failed;
					print "<br />No encontre datos para " . join(', ', @line_data) . "\n";
			...
			print "<br />Total Bills Generated: $bills_ok\nTotal Bills Failed: $bills_failed\n\n";
...
#############################################################################
	...
	&add_order_notes_by_type($id_orders,"Check Order","$Type");
...
#############################################################################
	...
	if($bill_data{'type'} eq 'Deposit'){
		$mod_currency .= "ID_accounts = '$bill_data{'deposit_id_accounts'}', ";
	}elsif($bill_data{'type'} eq 'Bill'){
		$mod_currency .= "ID_accounts = '$bill_data{'bill_id_accounts'}', ";
	}
	$qry_invoice = ($bill_data{'invoice'}) ? "Invoice='".$bill_data{'invoice'}."'," : "";
	my ($sth) = &Do_SQL("/*$cfg{'dbi_db'}*/ INSERT INTO sl_bills SET ID_vendors = '$bill_data{'id_vendors'}', Type = '$bill_data{'type'}', Currency = '$bill_data{'currency'}', Amount = '$bill_data{'amount'}', Memo = '$bill_data{'memo'}', $mod_currency $qry_invoice BillDate = '$bill_data{'date'}', DueDate = '$bill_data{'date'}', AuthBy = '1', PaymentMethod = 'Wire Transfer', Status = 'New', Date = '$bill_data{'date'}', Time = CURTIME(), ID_admin_users = '1';");
...
#############################################################################
	...
	for(1..$lines){
		...
		$id_segments = ($bill_data{'id_segments'}) ? $bill_data{'id_segments'} : '0';
		$deductible = ($bill_data{'deductible'}) ? $bill_data{'deductible'} : 'No';

		my $query = "/*  */ INSERT INTO sl_bills_expenses SET ID_bills = '$bill_data{'id_bills'}', Amount = '$amount', ID_accounts = '$id_accounts', ID_segments = $id_segments, Related_ID_bills_expenses = '$last_id', Deductible = '$deductible', Date = '$bill_data{'date'}', Time = CURTIME(), ID_admin_users = '1';";
		my ($sth) = &Do_SQL($query);
