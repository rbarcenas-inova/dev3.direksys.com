...
# --------------------------------------------------------
	...
	my ($sth) = &Do_SQL("SELECT COUNT(*) FROM sl_products WHERE ID_brands='$in{'id_brands'}' AND Status<>'Inactive'");
	$va{'matches'} = $sth->fetchrow;
	if ($va{'matches'}>0){
		(!$in{'nh'}) and ($in{'nh'}=1);
		$first = ($in{'nh'} - 1) * $usr{'pref_maxh'};			
		($va{'pageslist'},$qs) = &pages_list($in{'nh'},"/cgi-bin/mod/$usr{'application'}/dbman",$va{'matches'},$usr{'pref_maxh'});
		my ($sth) = &Do_SQL("SELECT * FROM sl_products WHERE ID_brands='$in{'id_brands'}' AND Status<>'Inactive' ORDER BY sl_products.Name DESC LIMIT $first,$usr{'pref_maxh'};");
