...
# Last Modified RB: 12/16/2010  16:58:30 -- parámetros sltv_itemshipping
	...
	## Flexipago
	$in{'flexipago'} = int($in{'flexipago'});
	($in{'flexipago'} = 1) unless ($in{'flexipago'});
...
# Created on: 1/1/2007 9:43AM
# Last Modified on:
# Last Modified by: Jos\x{0082} Ramirez Garcia
# Author: 
# Description : 
# Parameters :
...
# Last Modified on:
# Last Modified by:
# Author: Jos\x{0082} Ramirez Garcia
...
#############################################################################
#############################################################################
#   Function: validate_phone_blacklist
#
#       Es: Validate for cmd=validate_phone_blacklist
#       En: 
#
#
...
#############################################################################
	...
	if (!$in{'type'}){
		$error{'type'} = &trans_txt('invalid');
		++$errors;
	}

	if (!$in{'phone'}){
		$error{'phone'} = &trans_txt('invalid');
		++$errors;
	}	
