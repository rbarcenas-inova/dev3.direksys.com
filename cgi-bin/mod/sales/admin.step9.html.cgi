...
#######delete($cses{'id_orders'}); #for tests only
	...
	if($cses{'id_orders'} and $cses{'status9'} eq 'ok'){

		##########
		########## PAYMENT && ACCOUNTING VALIDATION
		##########
		my ($sth) = &Do_SQL("SELECT COUNT(*) FROM sl_orders_payments WHERE ID_orders = '$cses{'id_orders'}' AND Captured = 'Yes' AND CapDate = CURDATE() AND Status = 'Approved';");
		my($total_captured) = $sth->fetchrow();

		my ($sth) = &Do_SQL("SELECT COUNT(*) FROM sl_movements WHERE ID_tableused = '$cses{'id_orders'}' AND Status = 'Active';");
		my($total_movements) = $sth->fetchrow();

		if($total_captured or $total_movements){

			########
			######## A) Order has already payments or movements, We can't delete anything
			########
			#&Do_SQL("INSERT INTO sl_orders_notes SET Notes='Order Already Processed Tried to be Processed Again', ID_orders = $cses{'id_orders'},Type='High',Date=CURDATE(),Time=CURTIME(),ID_admin_users='$usr{'id_admin_users'}';");
			&add_order_notes_by_type($cses{'id_orders'},"Order Already Processed Tried to be Processed Again","High");

			$url = "/cgi-bin/mod/sales/admin?cmd=console_order&step=10&reprocessed=1&id_orders=" . $cses{'id_orders'};
			print "Location: $url\n\n";
			exit;
	...
	($cses{'status9cc'}) and ($cses{'status9'} = 'ok');
	...
	if (!$cses{'id_orders'}){
		...
		if($cfg{'orderstype'}==1){
			...
			if ($cses{'pay_type'} eq 'wu' or $cses{'pay_type'} eq 'mo' or $cses{'laytype'} eq 'mo' or $cses{'pay_type'} eq 'cod' or $cses{'pay_type'} eq 'rd' or ($cses{'laytype'} eq 'cc'  and $cses{'fppayments'}!=1)){
				...
				$query =~  s/DIDS7=''/DIDS7='$cses{'did'}'/ if ($cses{'did'});
				
				my ($sth) = &Do_SQL("INSERT INTO sl_orders SET $query Date=Curdate(),Time=CURTIME(),ID_admin_users='$usr{'id_admin_users'}'");
			...
			if ($cses{'pay_type'} eq 'cc' and $cses{'fppayments'}>1){
				...
				for ($p = $cses{'fppayments'}; $p >= 1; $p--){
					...
					for my $i(1..9){
						if( $cses{'pay_type'} eq 'cc' and ($cfg{'encrypt_cc'} and $cfg{'encrypt_cc'} == 1) ){
							if( $i == 3 ){
								...
								$query .= "PmtField3='".substr($cc_number, 0, 6)."xxxxxx".substr($cc_number, -4)."',";
							}elsif( $i == 4 ){
								$cc_date = &filter_values($cses{'pmtfield4'});
								$query .= "PmtField4='xxxx',";
							}elsif( $i == 5 ){
								$cc_cvn = &filter_values($cses{'pmtfield5'});
								$query .= (length($cc_cvn) == 4) ? "PmtField5='xxxx'," : "PmtField5='xxx',";
							}else{
					...
					chop($query);
					my ($sth) = &Do_SQL("INSERT INTO sl_orders_payments SET ID_orders='$cses{'id_orders'}',Type='$paytype{$cses{'pay_type'}}',$query ,Amount='$cses{'fppayment'.$p}',Paymentdate='$cses{'fpdate'.$p}', Date=CURDATE(),Time=CURTIME(),ID_admin_users='$usr{'id_admin_users'}'");
				...
				if ($cses{'pay_type'} eq 'wu' or $cses{'pay_type'} eq 'mo' or $cses{'laytype'} eq 'mo' or $cses{'pay_type'} eq 'cod' or ($cses{'laytype'} eq 'cc'  and $cses{'fppayments'}!=1)){
					...
					if($cses{'pay_type'} eq 'lay' and $cses{'fppayments'}!=1){
						...
						#######################
						#######################
						my ($sth) = &Do_SQL("INSERT INTO sl_orders_payments SET ID_orders='$cses{'id_orders'}',Type='$paytype{$cses{'pay_type'}}',$query ,Amount='$cses{'total_order'}',Status='Pending', Date=CURDATE(),Time=CURTIME(),ID_admin_users='$usr{'id_admin_users'}'");
						my $id_payments = $sth->{'mysql_insertid'};
					...
					my ($sth) = &Do_SQL("INSERT INTO sl_orders_payments SET ID_orders='$cses{'id_orders'}',Type='$paytype{$cses{'pay_type'}}',$query ,Amount='$cses{'total_order'}', Date=CURDATE(),Time=CURTIME(),ID_admin_users='$usr{'id_admin_users'}'");
					my $id_payments = $sth->{'mysql_insertid'};
			...
			for my $i(1..$cses{'items_in_basket'}){
				if ($cses{'items_'.$i.'_qty'}>0 and $cses{'items_'.$i.'_id'}>0){
					...
					## Ya con el porcentaje de descuento, calcula el costo de cada item
					if( $cses{'percent_disc'} ){
						$cses{'items_'.$i.'_discount'} += ( $cses{'percent_disc'}/100 ) * ($cses{'items_'.$i.'_fpprice'} * $cses{'items_'.$i.'_qty'});
						$cses{'items_'.$i.'_tax'} -= ( $cses{'percent_disc'}/100 ) * $cses{'items_'.$i.'_tax'};
					}
			...
			if($orderShipment){
				...
				$total_shp = $cfg{$config_values_prices{$cses{'pay_type'}}{$cses{'shp_type'}}};
				my $query = "
				select 
					id_orders_products,
					round( $total_shp  * ShpTax_percent,2) shptax
				from 
					sl_orders_products
				where 
					id_orders='$cses{'id_orders'}' and 
					ID_products not like '6%'
				order by 
					salePrice desc
				limit 1";
				my ($sth) = &Do_SQL($query);
				...
				&Do_SQL($query);
				$query = "update sl_orders_products set Shipping = '0', ShpTax =0 where id_orders_products !='$price->{'id_orders_products'}' and id_orders = '$cses{'id_orders'}' and ID_products not like '6%'";
				&Do_SQL($query);
		...
		## Se almacena la transacción de un coupon. 
		if( $cses{'id_coupons'} ){		
			&Do_SQL("INSERT INTO sl_coupons_trans SET ID_coupons='$cses{'id_coupons'}', ID_orders='$cses{'id_orders'}', Date=CURDATE() , Time=CURTIME() , ID_admin_users='$usr{'id_admin_users'}';");
		}
		...
		for my $i(1..$cses{'items_in_basket'}){
			if ($cses{'items_'.$i.'_qty'}>0 and $cses{'items_'.$i.'_id'}>0){
				...
				## Ya con el porcentaje de descuento calculado u obtendidose calcula el costo de cada item
				if( $cses{'percent_disc'} ){
					$cses{'items_'.$i.'_discount'} += ( $cses{'percent_disc'}/100 ) * ($cses{'items_'.$i.'_fpprice'} * $cses{'items_'.$i.'_qty'});
					$cses{'items_'.$i.'_tax'} -= ( $cses{'percent_disc'}/100 ) * $cses{'items_'.$i.'_tax'};
				}
		...
		if($cses{'pay_type'} eq 'cc'){
			...
			my ($sth) = &Do_SQL("DELETE FROM sl_orders_payments WHERE ID_orders='$cses{'id_orders'}'");
			my ($sth) = &Do_SQL("DELETE FROM sl_orders_cardsdata WHERE ID_orders='$cses{'id_orders'}'");
			...
			for ($p = $cses{'fppayments'}; $p >= 1; $p--){
				...
				($p == 1) and ($pd_price > 0) and ($in{'postdated'} or $cses{'postdated'}) and ($cses{'total_order'}-$cses{'shipping_total'}-$cses{'tax_total'} >  350) and ($cses{'fppayment'.$p} -= $pd_price);
				chop($query);
				my ($sth) = &Do_SQL("INSERT INTO sl_orders_payments SET ID_orders='$cses{'id_orders'}',Type='$paytype{$cses{'pay_type'}}',$query ,Amount='$cses{'fppayment'.$p}',Paymentdate='$cses{'fpdate'.$p}', Date=Curdate(),Time=NOW(),ID_admin_users='$usr{'id_admin_users'}'");
				my $id_payments = $sth->{'mysql_insertid'};
			...
			my ($sth) = &Do_SQL("INSERT INTO sl_orders_payments SET ID_orders='$cses{'id_orders'}',Type='$paytype{$cses{'pay_type'}}', $query Amount='$cses{'total_order'}',Paymentdate=DATE_ADD(CURDATE(),INTERVAL 1 DAY), Date=CURDATE(),Time=CURTIME(),ID_admin_users='$usr{'id_admin_users'}'");
		}
		...
		if($cses{'pay_type'} eq 'cc'){
			...
			if ($sth->fetchrow>0){
				...
				if ($riskorder ne 'OK'){
					...
					&Do_SQL("INSERT INTO sl_orders_notes SET Notes='Risk Order\n$riskorder', ID_orders = $cses{'id_orders'},Type='High',Date=Curdate(),Time=NOW(),ID_admin_users='$usr{'id_admin_users'}'");
	...
	if($cfg{'allorders_onepayment'} and ($cses{'pay_type'} eq 'cc' or $cses{'laytype'} eq 'cc') ) {
		...
		if($amount > 0 and $idop){
			...
			if(!$cfg{'use_points'} and !$in{'activar_puntos'} ){
		...
		if($cses{'pay_type'} eq 'cc' or $cses{'laytype'} eq 'cc'){
			...
			if (($in{'postdated'} or $cses{'postdated'}) && $cfg{'postdatedbutton'}){
				...
				if( $cfg{'allorders_onepayment'} ) {
					...
					$query = '';
					for my $i(1..9){
						if( $paytype{$cses{'pay_type'}} eq 'cc' and ($cfg{'encrypt_cc'} and $cfg{'encrypt_cc'} == 1) ){
							if( $i == 3 ){
								$cc_number = &filter_values($cses{'pmtfield3'});
								$query .= "PmtField3='".substr($cc_number, 0, 6)."xxxxxx".substr($cc_number, -4)."',";
							}elsif( $i == 4 ){
					...
					&Do_SQL("INSERT INTO sl_orders_payments SET ID_orders='$cses{'id_orders'}',Type='$paytype{$cses{'pay_type'}}',$query,Amount='$pd_price',Paymentdate=CURDATE(), Date=CURDATE(),Time=CURTIME(),ID_admin_users='$usr{'id_admin_users'}'");
					my ($stlid) = &Do_SQL("SELECT LAST_INSERT_ID()");
					$idordpay = $stlid->fetchrow;
					$id_payments = $idordpay;
				...
				if ($status eq 'OK'){
					...
					my ($sth)=&Do_SQL("INSERT INTO sl_orders_notes SET ID_orders='$va{'id_orders'}', Notes='La orden se pasa a Void por fallo en el cobro del posdated fee', Type='Low', Date=CURDATE(), Time=CURTIME(), ID_admin_users='$usr{'id_admin_users'}'");
					&add_order_notes_by_type_admin($va{'id_orders'},"La orden se pasa a Void por fallo en el cobro del posdated fee","Low");
			...
			if ($cses{'pay_type'} eq 'cod' and ($in{'postdated'} or $cses{'postdated'}) and $cfg{'postdatedbutton'}){
				...
				if(!$in{'days'} or $in{'days'} == 0) {
					$in{'days'} = ($in{'dayspay'}) ? $in{'dayspay'} : 1;
				}
				###
				...
				&Do_SQL("INSERT INTO sl_orders_notes SET ID_orders='$cses{'id_orders'}',ID_orders_notes_types=1, Notes = CONCAT('La fecha de hoy es: ',CURDATE(),' y los días elegidos para postfechada son: $in{'days'}'), Type='Low', Date=CURDATE(), Time=CURTIME(), ID_admin_users='$usr{'id_admin_users'}'");
	...
	## Debug Only
	if ($cses{'debug_lines'}){
		&Do_SQL("INSERT INTO sl_debug (cmd, id, log, Date, Time, ID_admin_users) VALUES ('console_order', '$cses{'id_orders'}', '".&filter_values($cses{'debug_lines'})."', CURDATE(), CURTIME(), '$usr{'id_admin_users'}');");
	}
	## --------------------------------------------------------
