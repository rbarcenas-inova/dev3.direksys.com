...
# Last Time Modified By RB on 2/16/10 2:06 PM : Se cambia para direccionar a Orders y se revisa que funcione para Layaway y COD
	...
	if($in{'ptype'}){
		...
		if($in{'ptype'} eq 'Layaway'){
		    ...
		    $cad = qq|<td class='gcell_$due' align='center' onClick='trjump("/cgi-bin/mod/ecom/dbman?cmd=opr_orders&search=listall&sb=id_orders&status=New&statusPay=Layaway Due&so=DESC&ptype=$in{'ptype'}")'>Due</td>|;
		}
		...
		$cad = qq|
							<table width="100%" border="0" cellspacing="0" cellpadding="0" class="gtab" align="center">
				  			<tr>
							    ...
							    <td class='gcell_$gen' align='center' onClick='trjump("/cgi-bin/mod/ecom/dbman?cmd=opr_orders&search=listall&sb=id_orders&so=DESC&ptype=$in{'ptype'}")'>All</td>
							    <td class='gcell_$new' align='center' onClick='trjump("/cgi-bin/mod/ecom/dbman?cmd=opr_orders&search=listall&sb=id_orders&status=New&so=DESC&ptype=$in{'ptype'}")'>New</td>
							    <td class="gcell_$proc" align="center" onClick='trjump("/cgi-bin/mod/ecom/dbman?cmd=opr_orders&search=listall&sb=id_orders&status=Processed&so=DESC&ptype=$in{'ptype'}")'>Processed</td>
							    $cad
							    <td class="gcell_$can" align="center" onClick='trjump("/cgi-bin/mod/ecom/dbman?cmd=opr_orders&search=listall&sb=id_orders&status=Cancelled&so=DESC&ptype=$in{'ptype'}")'>Cancelled</td>
							  </tr>
							</table>|;
	}
}
...
# Last Time Modified By RB on 2/16/10 2:09 PM : Se redirecciona a Orders debido al desuso de Preorders 
	...
	$cad = qq|
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="gtab" align="center">
			  			<tr>
						    <td width="30%" align="center"></td>
						    <td class='gcell_$td' align='center' onClick='trjump("/cgi-bin/mod/ecom/admin?cmd=orders_payments&range=today")'>Today</td>
						    <td class='gcell_$trd' align='center' onClick='trjump("/cgi-bin/mod/ecom/admin?cmd=orders_payments&range=3d")'>Last  3 Days</td>
						    <td class="gcell_$sd" align="center" onClick='trjump("/cgi-bin/mod/ecom/admin?cmd=orders_payments&range=7d")'>Last Week</td>
						    <td class="gcell_$fd" align="center" onClick='trjump("/cgi-bin/mod/ecom/admin?cmd=orders_payments&range=15d")'>Last 2 Weeks</td>
						    <!--<td class='gcell_$thd' align='center' onClick='trjump("/cgi-bin/mod/ecom/admin?cmd=orders_payments&range=30d")'>Last Month</td>-->
						  </tr>
						</table>|;
}
...
# Last Modified by: MCC C. Gabriel Varela S: Se toma en cuenta lang de $usr
		...
		my $dir = $cfg{'path_templates'} . '/app/ecom/email_templates';
#		$dir =~	s/\[lang\]/en/;
