...
#
	...
	if ($va{'matches'}>0){
		...
		while ($rec = $sth->fetchrow_hashref){
			$d = 1 - $d;
			$va{'searchresults'} .= qq|
			<tr bgcolor='$c[$d]' onmouseover='m_over(this)' onmouseout='m_out(this)' onclick="trjump('/cgi-bin/mod/ecom/admin?cmd=opq_products_view&view=$rec->{'ID_products'}&tab=6#tabs')">
	...
	$va{'page_title'} = trans_txt("pagesuppliers");
...
#
	...
	my $str_template = '';
	delete($va{'pname'});
	($va{'from_run_daily'}) and ($str_template = 'app/administration/');
	...
	print "Content-type: text/html\n\n";
	$va{'page_title'} = trans_txt("eccomerce");
...
# Last Time modified by RB on 01/21/2011 :  Se agrego status de la orden y Fecha del mensaje
	...
	if($in{'get_status'} eq 'new'){
	    ...
	    $va{'new'}='cell_on'; $va{'answer'}='cell_off'; $va{'continue'}='cell_off'; $va{'close'}='cell_off'; $va{'reopen'}='cell_off'; $va{'void'}='cell_off';
	}elsif($in{'get_status'} eq 'answer'){
	    $modquery = " AND Status = 'Answered' ";
	    $va{'new'}='cell_off'; $va{'answer'}='cell_on'; $va{'continue'}='cell_off'; $va{'close'}='cell_off'; $va{'reopen'}='cell_off'; $va{'void'}='cell_off';
	}elsif($in{'get_status'} eq 'continue'){
	    $modquery = " AND Status = 'Continued' ";
	    $va{'new'}='cell_off'; $va{'answer'}='cell_off'; $va{'continue'}='cell_on'; $va{'close'}='cell_off'; $va{'reopen'}='cell_off'; $va{'void'}='cell_off';
	}elsif($in{'get_status'} eq 'close'){
	    $modquery = " AND Status = 'Closed' ";
	    $va{'new'}='cell_off'; $va{'answer'}='cell_off'; $va{'continue'}='cell_off'; $va{'close'}='cell_on'; $va{'reopen'}='cell_off'; $va{'void'}='cell_off';
	}elsif($in{'get_status'} eq 'reopen'){
	    $modquery = " AND Status = 'Reopened' ";
	    $va{'new'}='cell_off'; $va{'answer'}='cell_off'; $va{'continue'}='cell_off'; $va{'close'}='cell_off'; $va{'reopen'}='cell_on'; $va{'void'}='cell_off';
	}elsif($in{'get_status'} eq 'void'){
	    $modquery = " AND Status = 'Void' ";
	    $va{'new'}='cell_off'; $va{'answer'}='cell_off'; $va{'continue'}='cell_off'; $va{'close'}='cell_off'; $va{'reopen'}='cell_off'; $va{'void'}='cell_on';
	}
	...
	if ($va{'matches'}>0){
		...
		while ($rec = $sth->fetchrow_hashref){
			...
			($rec->{'ID_orders'} > 0) and ($linkorder = qq| <a href="/cgi-bin/mod/ecom/dbman?cmd=opr_orders&view=$rec->{'ID_orders'}">$rec->{'ID_orders'}</a>|);
			($rec->{'ID_orders'} == 0) and ($linkorder = 'N/A');
			$d = 1 - $d;
			$va{'searchresults'} .= qq|
			<tr bgcolor='$c[$d]'>
				<td class="smalltext" nowrap valign="top">$customer (<a href="/cgi-bin/mod/ecom/dbman?cmd=opr_customers&view=$rec->{'ID_customers'}">$rec->{'ID_customers'}</a>)</td>
				<td class="smalltext" nowrap valign="top">&nbsp;</td>
				<td class="smalltext" nowrap valign="top">$linkorder</td>
				<td class="smalltext" nowrap valign="top">&nbsp;</td>
				<td class="smalltext"><a href="/cgi-bin/mod/ecom/admin?cmd=$in{'cmd'}&get_status=$in{'get_status'}&view=$rec->{'ID_customers_support'}">$rec->{'shortquestion'} ...</a></td>
				<td class="smalltext" nowrap valign="top">&nbsp;</td>
	...
	print "Content-type: text/html\n\n";
	$va{'page_title'} = trans_txt("eccommerce - techsupport");
...
# Last Time modified by RB on 01/21/2011 :  Se agrego status de la orden
	...
	if ($va{'matches'}>0){
		...
		while ($rec = $sth->fetchrow_hashref){
			...
			$va{'searchresults'} .= qq|
			<tr bgcolor='$c[$d]'>
				<td class="smalltext" nowrap valign="top">$customer (<a href="/cgi-bin/mod/ecom/dbman?cmd=$linkcustomer&view=$rec->{'ID_customers'}">$rec->{'ID_customers'}</a>)</td>
				<td class="smalltext" nowrap valign="top">&nbsp;</td>
				<td class="smalltext" nowrap valign="top">$linkorder</td>
				<td class="smalltext" nowrap valign="top">&nbsp;</td>
...
# Last time Modified By RB on 10/07/2011: Se agrega Amazon
	...
	if($in{'id_customers'} ne '' and !$in{'action'}){
		...
		$in{'email'}=$rec->{'Email'};
		print "Content-type: text/html\n\n";
		$va{'page_title'} = trans_txt("pageadministration");
...
# 			$lastid_cus=217127;
			...
			if($in{'type'} =~ /PayPal|Google|Amazon/){
				&order_deposit($lastid_ord,$lastid_payment);
				$status='OK';
			}else{
				if(!$va{'from_run_daily'} == 1){
			...
			if($status =~ /ok/i){
				$va{'message'}='The payment was succesfully authorized';
			}else{
				$va{'message'}= $statmsg;
			}

			$va{'message'}="The order and customer have been created: Order ID <a href=\"/cgi-bin/mod/ecom/dbman?cmd=opr_orders&view=$lastid_ord\">$lastid_ord</a>, Customer ID <a href=\"$script_url?cmd=opr_customers&view=$lastid_cus\">$lastid_cus</a><br>$va{'message'}";
