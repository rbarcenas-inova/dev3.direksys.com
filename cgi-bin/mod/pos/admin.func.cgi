	...
	if ($in{'action'} eq 'add_to_basket'){
		...
		if ($in{'id_products'}>0  and length($in{'id_products'}) == 6){
			...
			if ($sth->fetchrow()>0){
				...
				my ($sth) = &Do_SQL("SELECT * $modq FROM sl_products_prices WHERE ID_products = '$in{'id_products'}' $add_sql ORDER BY PayType,FP DESC,Price DESC;");
				my $recs = 0;
...
# --------------------------------------------------------
	...
	for (0 .. $rows-1) {
		...
		$page .= qq|		    <tr onmouseover='m_over(this)' onmouseout='m_out(this)' bgcolor='$c[$d]' onClick="trjump('/cgi-bin/mod/pos/admin?cmd=console_order&id_products=$tmp{$db_cols[0]}&action=search&step=2')">\n|;
		for (0..$#headerfields){
			if ($headerfields[$_] =~ /([^:]+):([^\.]+)\.([^\.]+)/){
				## 1)DB  2)ID  3)Name
		...
		for my $i(1..$cses{'items_in_basket'}){
			...
			if ($cses{'items_'.$i.'_qty'}>0 and $cses{'items_'.$i.'_id'}>0){
				...
				$output .= qq|
					<tr>
					   <td  nowrap>
					   <a href='/cgi-bin/mod/pos/admin?cmd=console_order&step=2&drop=$i'><img src='$va{'imgurl'}/$usr{'pref_style'}/b_drop.png' title='Drop' alt='' border='0'></a>
					   		</td>
					   <td >$cses{'items_'.$i.'_qty'} x</td>
					   <td  onClick="trjump('/cgi-bin/mod/pos/admin?cmd=console_order&step=2&action=search&id_products=|.substr($cses{'items_'.$i.'_id'},3,6).qq|&choice1=$rec->{'choice1'}&choice2=$rec->{'choice2'}&choice3=$rec->{'choice3'}&choice4=$rec->{'choice4'}')">|.substr($desc[$i],0,10).qq|</td>
			...
			$output .= qq|
				...
				<tr>
				   <td colspan="4" align="center"><br><br>
				    <a href="/cgi-bin/mod/pos/admin?cmd=console_order&step=3"	class=button>CREAR ORDEN</a>
				    <br><br>
				    </td>
				</tr>\n|;
			return $output;
...
# Last Time Modified by RB on 07/25/2013 : Se agrego calculo de multiprice basado en sl_products_prices
	...
	##ADG ->
	# impuesto calculado por estado o ciudad solo mexico (formateado como 0.16)
			...
			if ($cses{'items_'.$i.'_qty'}>0 and $cses{'items_'.$i.'_id'}>0){
				...
				$va{'itemsproducts_list'} .= qq|
						<tr  onmouseover='m_over(this)' onmouseout='m_out(this)'>
							<td class="smalltext">|;
				$va{'itemsproducts_list'} .= qq|<a href='/cgi-bin/mod/pos/admin?cmd=console_order&step=$in{"step"}&drop=$i#tabs'><img src='$va{'imgurl'}/$usr{'pref_style'}/b_drop.png' title='Drop' alt='' border='0'></a>| if ($in{'step'} =~ /8/);
				$va{'itemsproducts_list'} .= qq|			
									<a href="/cgi-bin/mod/pos/admin?cmd=console_order&step=2&action=search&id_products=|.substr($cses{'items_'.$i.'_id'},3,6).qq|&choice1=$rec->{'choice1'}&choice2=$rec->{'choice2'}&choice3=$rec->{'choice3'}&choice4=$rec->{'choice4'}"><img src='[va_imgurl]/[ur_pref_style]/b_view.gif' title='View' alt='' border='0'></a>
				...
				if($rec->{'SalesPrice'} eq 'Fixed'){
					...
					$va{'itemsproducts_list'} .= qq|
						<tr  onmouseover='m_over(this)' onmouseout='m_out(this)'>
							...
							<a href="/cgi-bin/mod/pos/admin?cmd=console_order&step=2&action=search&id_products=|.substr($cses{'servis_'.$i.'_id'},5).qq|"><img src='[va_imgurl]/[ur_pref_style]/b_view.gif' title='View' alt='' border='0'></a>
							$serviceid</td>
...
# Last Time Modified by _RB_ on 07/24/2013: Se agrega verificacion de 
	...
	if($cses{'pay_type'}){
		...
		if(!$avpaytypes && !$onlyservices){
			...
			print "<script>location.href='/cgi-bin/mod/pos/admin?cmd=console_order&step=3';</script>";
		} else {
			return 1;
		}
	} else {
	...
	if($cses{'pay_type'} eq "cod" or $cses{'pay_type'} eq "rd"){
		...
		if($in{'step'} =~ /5/ && $cses{'id_customers'}){
			...
			while ($rec = $sth->fetchrow_hashref){
				...
				if(uc($cfg{'country'}) eq $cfg{'country'})  {
					$in{'shp_address1'} = $rec->{'Address1'};
					$in{'shp_address2'} = $rec->{'Address2'};
					$in{'shp_address3'} = $rec->{'Address3'};
...
# Last Modified by RB on 09/05/2012: Se agrega opcion para express delivery basado en configuracion. Para TDC
	...
	($cses{'shp_type'} < 3) and ($va{'shp_notcod_display'} = 'display:table') and ($va{'shp_cod_display'} = 'display:none');
	($cses{'shp_type'} > 2) and ($va{'shp_notcod_display'} = 'display:none') and ($va{'shp_cod_display'} = 'display:table');
	...
	if ($cses{'items_in_basket'} > 0 || $cses{'servis_in_basket'} > 0){
		...
		PRODUCTS:for my $i(1..$cses{'items_in_basket'}){
			...
			if ($cses{'items_'.$i.'_qty'}>0 and $cses{'items_'.$i.'_id'}>0){				
				...
				if($cses{'shp_type'} == 1){
					if($rec->{'free_shp_opt'} eq 'Yes' and !$cses{'items_'.$i.'_shpf'}){			
						$va{'shipments_options_standard'} .= qq|<a href='/cgi-bin/mod/pos/admin?cmd=console_order&step=$step&dropshp=$i#tabs'>
													<img src='$va{'imgurl'}/$usr{'pref_style'}/checkmark_off.gif' title='Free Shipping' alt='Free Shipping' border='0'></a>|;
					}

					if($rec->{'free_shp_opt'} eq 'Yes' and $cses{'items_'.$i.'_shpf'}){
						$va{'shipments_options_standard'} .= qq|<a href='/cgi-bin/mod/pos/admin?cmd=console_order&step=$step&activateshp=$i#tabs'>
				...
				if($cses{'shp_type'} == 2){
					if($rec->{'free_shp_opt'} eq 'Yes' and !$cses{'items_'.$i.'_shpf'}){			
						$va{'shipments_options_express'} .= qq|<a href='/cgi-bin/mod/pos/admin?cmd=console_order&step=$step&dropshp=$i#tabs'>
													<img src='$va{'imgurl'}/$usr{'pref_style'}/checkmark_off.gif' title='Free Shipping' alt='Free Shipping' border='0'></a>|;
					}

					if($rec->{'free_shp_opt'} eq 'Yes' and $cses{'items_'.$i.'_shpf'}){
						$va{'shipments_options_express'} .= qq|<a href='/cgi-bin/mod/pos/admin?cmd=console_order&step=$step&activateshp=$i#tabs'>
													<img src='$va{'imgurl'}/$usr{'pref_style'}/checkmark.gif' title='Free Shipping' alt='Free Shipping' border='0'></a>|;
					}
				}
				...
				if($cses{'shp_type'} == 3){
					if($rec->{'free_shp_opt'} eq 'Yes' and !$cses{'items_'.$i.'_shpf'}){			
						$va{'shipments_options_scheduled'} .= qq|<a href='/cgi-bin/mod/pos/admin?cmd=console_order&step=$step&dropshp=$i#tabs'>
													<img src='$va{'imgurl'}/$usr{'pref_style'}/checkmark_off.gif' title='Free Shipping' alt='Free Shipping' border='0'></a>|;
					}

					if($rec->{'free_shp_opt'} eq 'Yes' and $cses{'items_'.$i.'_shpf'}){
						$va{'shipments_options_scheduled'} .= qq|<a href='/cgi-bin/mod/pos/admin?cmd=console_order&step=$step&activateshp=$i#tabs'>
													<img src='$va{'imgurl'}/$usr{'pref_style'}/checkmark.gif' title='Free Shipping' alt='Free Shipping' border='0'></a>|;
					}
				}	
...
#-----------------------------------------
	...
	while ($rec = $sth->fetchrow_hashref()){
		...
		$output .= qq|
			<tr  onmouseover='m_over(this)' onmouseout='m_out(this)' bgcolor='$c[$d]' onClick="trjump('/cgi-bin/mod/pos/admin?cmd=console_order&step=2&action=search&filter=proghoy&ID_mediacontracts=$rec->{'ID_mediacontracts'}')">
...
#-----------------------------------------
	if ($cses{'prodfam'}){
		return qq|
			<a href="/cgi-bin/mod/pos/admin?cmd=console_order&step=2&action=search&name=$cses{'prodfam'}">
...
#-----------------------------------------
	...
	#					FROM sl_noninv_related inner join sl_noninv on (sl_noninv.ID_noninv=sl_noninv_related.ID_noninv) WHERE sl_noninv_related.Status='Active' and sl_noninv.Status='Active' and sl_noninv_related.ID_products in (".$insql.")					
			...
			while ($rec = $sth->fetchrow_hashref){
				...
				($rec->{'type'} ne 'ni') and ($link = '<a href="/cgi-bin/mod/pos/admin?cmd=console_order&id_products='.substr($rec->{'ID_products'},3).'&action=search&step=2"><img src="[va_imgurl]/[ur_pref_style]/b_view.gif" title="View" alt="" border="0"></a>');
				...
				$va{'searchresults'} .="<tr onmouseover='m_over(this)' onmouseout='m_out(this)' bgcolor='$c[$d]' onclick=\"trjump('/cgi-bin/mod/pos/admin?cmd=console_order&step=8&tab=1&addrelprod".$rec->{'type'}."=1&relid=".$relid."&id_sku_products=".$rec->{'ID_products'}."')\">
		...
		while ($rec = $sth->fetchrow_hashref)	{
			...
			$cadimages=&show_image_in_page($rec->{'ID_products'})."<br>";
			$cadimages=""if($cadimages=~/No hay images/);
			$va{'searchresults'} .="<tr onmouseover='m_over(this)' onmouseout='m_out(this)' bgcolor='$c[$d]' onclick=\"trjump('/cgi-bin/mod/pos/admin?cmd=console_order&step=8&tab=2&addrelprod=1&relid=-1&id_sku_products=100".$rec->{'ID_products'}."')\">
		...
		while ($rec = $sth->fetchrow_hashref)	{
			...
			$cadimages=&show_image_in_page($rec->{'ID_products'})."<br>";
			$cadimages=""if($cadimages=~/No hay images/);
			$va{'searchresults'} .="<tr onmouseover='m_over(this)' onmouseout='m_out(this)' bgcolor='$c[$d]' onclick=\"trjump('/cgi-bin/mod/pos/admin?cmd=console_order&step=8&tab=3&addrelprod=1&id_sku_products=100".$rec->{'ID_products'}."')\">
																<td>$cadimages".&format_sltvid('100'.$rec->{'ID_products'})."</td>
																<td>".$rec->{'Name'}."$choices</td>
																<td>\$".$rec->{'SPrice'}."</td>
															</tr>";
		...
		if ($va{'matches'}>0){#GV Modifica 21abr2008 Se cambia sl_services por sl_services #GV Modifica 21abr2008 Se cambia ID_services por ID_services
			...
			while ($rec = $sth->fetchrow_hashref){
				...
				$va{'searchresults'} .= "<tr onmouseover='m_over(this)' onmouseout='m_out(this)' bgcolor='$c[$d]' onclick=\"trjump('/cgi-bin/mod/pos/admin?cmd=console_order&step=8&tab=4&ser=1&id_services=$rec->{'ID_services'}')\">\n";						
...
#############################################################################
#############################################################################
#   Function: loadDataPos
#
#       Es: Obtiene datos de usuario, warehouse de pos
#       
#
#
#    Created on: 13/01/2016 9:25
#
...
#############################################################################
	my %data;
	my ($warehouse)= &Do_SQL("select Subcode from sl_vars where sl_vars.vname='pos_config_$usr{'id_admin_users'}' and VValue='$usr{'id_admin_users'}'")->fetchrow();
	$pos_users = $cfg{'pos_users'};
	$data{'id_warehouse'} = $warehouse;
	$data{'id_admin_users'} = $usr{'id_admin_users'};
	return \%data;
}

sub checkPaymentOrder{
	my ($id_orders) = @_;
	$sql = "SELECT * FROM sl_orders_payments WHERE ID_orders = '$id_orders' AND Captured = 'Yes' AND CapDate IS NOT(NULL)";
}
