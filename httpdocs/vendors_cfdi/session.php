...
if (isset($_COOKIE[$ck_name]) || $sid != '') {    
    ...
    $res2 = mysql_fetch_assoc(mysql_query("SELECT IF(COUNT(*)>0,'Disallow','Allow') Entra FROM admin_groups_perms WHERE (ID_admin_groups IN (SELECT admin_users_groups.ID_admin_groups FROM admin_users_groups LEFT JOIN admin_groups ON admin_groups.ID_admin_groups=admin_users_groups.ID_admin_groups  WHERE admin_users_groups.ID_admin_users='".$usr['id_admin_users']."' AND Status='Active') OR ID_admin_groups = 0) AND application='admin' AND command='mod_cfdi';"));
