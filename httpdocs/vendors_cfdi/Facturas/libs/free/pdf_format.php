...
function pdf($id_invoices,$xml_string='',$type="show",$invoiceInfo = array(), $tt){
	...
	$factura['empresa_domiclio']	= strtoupper($xml_factura_emisor["DomicilioFiscal"]["@atributos"]["calle"])." ".$xml_factura_emisor["DomicilioFiscal"]["@atributos"]["noExterior"];
	...
	$factura['empresa_domiclio']	.= strtoupper($xml_factura_emisor["DomicilioFiscal"]["@atributos"]["pais"]. ", ".$xml_factura_emisor["DomicilioFiscal"]["@atributos"]["estado"] ." CP ").$xml_factura_emisor["DomicilioFiscal"]["@atributos"]["codigoPostal"];	
	...
	$factura['cliente_domicilio']	= strtoupper($xml_factura_cliente["DomicilioFiscal"]["@atributos"]["calle"])." ".$xml_factura_cliente["DomicilioFiscal"]["@atributos"]["noExterior"];
	...
	$band_iva0 = true;
	foreach( $impuestos["Retenciones"]["Retencion"]["@atributos"] As $impuesto ){
		$pdf->SetFont('Arial', '', 6.5);
		$pdf->SetXY($x+150, $y);
		$tasa = round(($impuesto["importe"]/$totales['subtotal']) * 100, 2);
		$pdf->Cell(20, 3.8, $impuesto["impuesto"].' '.$tasa.'%:', 0, 0, 'R');
		$pdf->SetFont('Arial', 'B', 6.5);
		$pdf->SetXY($x+170, $y);
		$pdf->Cell(25, 3.8, '$ '.number_format($impuesto["importe"], 2, '.', ','), 0, 0, 'R');
		$y += 3.8;
	...
	if( count($impuestos["Traslados"]["Traslado"]["@atributos"]) == 1 && $band_iva0 ){
		$pdf->SetFont('Arial', '', 6.5);
		$pdf->SetXY($x+150, $y);
	...
	$h_totals = ($y - $y_totals_ini) - 3;
	$pdf->Rect($x, $y_totals_ini, 195, $h_totals);
	$pdf->Line($x+147, $y_totals_ini, $x+147, $y_totals_ini+$h_totals);
	$pdf->Line($x+170, $y_totals_ini, $x+170, $y_totals_ini+$h_totals);
	...
	$y += 4;
	$pdf->SetXY($x, $y);
	$pdf->Cell(100, 4, 'REGIMEN FISCAL: '.utf8_decode($factura['regimen_fiscal']));
	$y += 4;
	$pdf->SetXY($x, $y);
	$pdf->Cell(100, 4, utf8_decode('LUGAR DE EXPEDICIÓN: ').utf8_decode($factura['lugar_expedicion']));
	...
	elseif($type=='download')		
		$pdf->Output("$factura[folio_fiscal].pdf",'D');
	elseif($type=='string')
		return $pdf->Output("$factura[folio_fiscal].pdf",'S');
}
...
function invoice_header($pdf, $factura){
	...
	$pdf->SetXY($x, $y);
	$pdf->Cell(90, 4, utf8_decode($factura['empresa_nombre']));
	
	$y += 4;
	$pdf->SetXY($x, $y);
	$pdf->Cell(60, 4, "RFC: ".utf8_decode($factura['empresa_rfc']));
	
	$pdf->SetFont('Arial', '', 6);
	$y += 4;
	$pdf->SetXY($x, $y);	
	$pdf->MultiCell(78, 3, utf8_decode($factura['empresa_domiclio']));
	...
	$pdf->SetFont('Arial', '', 8);	
	$pdf->SetXY($x, $y);
	$pdf->Cell(32.5, 4.5, utf8_decode('MONEDA'), 1, 0, 'C', 'true');
	$pdf->Cell(32.5, 4.5, utf8_decode('TIPO DE CAMBIO'), 1, 0, 'C', true);
	$y += 4.5;
	$pdf->SetTextColor(0);
	$pdf->SetXY($x, $y);
	$pdf->Cell(32.5, 4.5, $factura["moneda"], 1, 0, 'C');	
	$pdf->Cell(32.5, 4.5, $factura["tipocambio"], 1, 0, 'C');
	...
	if( strtoupper($factura['tipo_comprobante']) == 'INGRESO' ){
		$pdf->Cell(130, 5, utf8_decode('FACTURA'), '1', 0, 'C');
	}else{
		$pdf->Cell(130, 5, utf8_decode('NOTA DE CRÉDITO'), '1', 0, 'C');
	}
	...
	$pdf->SetXY($x, $y);
	$pdf->Cell(70, 5, utf8_decode('INFORMACIÓN DEL RECEPTOR'));
	$y += 6;
	// Cliente
	$pdf->SetFont('Arial', '', 7);
	$pdf->SetXY($x, $y);
