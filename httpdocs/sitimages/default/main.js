...
function textOnly( e ){
	...
	return /[\da-zA-Z\s\u00e1\u00e9\u00ed\u00f3\u00fa\u00c1\u00c9\u00cd\u00d3\u00da\u00f1\u00d1]/.test(String.fromCharCode(keynum));
}
function verifyTextOnly( field ){
	texto = $(field).val();
	if(texto.match(/^[a-zA-Z0-9.\s\u00e1\u00e9\u00ed\u00f3\u00fa\u00c1\u00c9\u00cd\u00d3\u00da\u00f1\u00d1]*$/) == null)
	{
		alert("Este campo solo debe contener Numeros, Letras y puntos.");
		field.focus();
	}
}
function letterOnly( e ){
	var keynum = window.event ? window.event.keyCode : e.which;
	if ((keynum == 8) || (keynum == 46) || (keynum == 99)||(keynum == 118)||(keynum == 0)) return true;
	return /[a-zA-Z\s\u00e1\u00e9\u00ed\u00f3\u00fa\u00c1\u00c9\u00cd\u00d3\u00da\u00f1\u00d1]/.test(String.fromCharCode(keynum));
}
function verifyLetterOnly( field ){
	texto = $(field).val();
	if(texto.match(/^[a-zA-Z.\s\u00e1\u00e9\u00ed\u00f3\u00fa\u00c1\u00c9\u00cd\u00d3\u00da\u00f1\u00d1]*$/) == null)
	{
		alert("Este campo solo debe contener Letras y puntos.");
		field.focus();
	}
}
function phoneNumberOnly( e ){
	var keynum = window.event ? window.event.keyCode : e.which;
	if ((keynum == 8) || (keynum == 99)||(keynum == 118)||(keynum == 0)) return true;
	return /[\d]/.test(String.fromCharCode(keynum));
}
function verifyPhoneNumbersOnly( field ){
	texto = $(field).val();
	if(texto.match(/^[\d]*$/) == null)
	{
		alert("Este campo solo debe contener Numeros");
		field.focus();
	}
}
...
function verifyNumbersOnly( field ){
	texto = $(field).val();
	if(texto.match(/^[\d.]*$/) == null)
	{
		alert("Este campo solo debe contener Numeros");
...
function m_over(o){
	if (typeof(o.style) != 'undefined'){
		...
		//o.style.backgroundColor = '#9dc130';
		o.style.border = '';
...
function prnwin(link) {
	...
	var w = $( window ).width();
	var h = $( window ).height();
	win = window.open("","extra","toolbar=no,width="+w+",height="+h+",directories=no,status=no,scrollbars=yes,resizable=yes,menubar=no");
...
function helponoff() {
	...
	var spans = document.getElementsByTagName('span');
	var numspans = spans.length;
	//var this_img = document.getElementById('img_voxhelp');
	var this_img = document.getElementById('helpleg_img');

	if (helpleg.innerHTML == 'Off'){
		helpleg.innerHTML="On";
		this_img.innerHTML = '<img id="img_voxhelp" src=/sitimages/app_bar/mod/helpOn.png width="74px" height="30px" style="cursor:pointer;">';
		...
		//this_img.src = '/sitimages/app_bar/mod/helpOff.png';
		this_img.innerHTML = '<img id="img_voxhelp" src=/sitimages/app_bar/mod/helpOff.png width="74px" height="30px" style="cursor:pointer;">';
...
function confirm_changeto_wholesale(script_url,cmd,id_orders){
	var answer = confirm("Really want to change order "+id_orders+" to Wholesale?");
	if(answer){
		window.location.href = script_url+"?cmd="+cmd+"&view="+id_orders+"&wholesale=1";
	}else{
		return false;
	}
}
...
function chk_all(){
	for (var i=0; i<self.document.sitform.length; i++){
		if (self.document.sitform.elements[i].type=='checkbox'){
			self.document.sitform.elements[i].click();;
		}
	}
}
...
var radioVal = function(e){
	if(e.checked === true){
		document.querySelector('[data-name="notestype"]').name = "notestype2";
	}
}
var getMsg = function(e){
	var v = window.dataCollection[e.options[e.selectedIndex].dataset.category].content[e.options[e.selectedIndex].dataset.element].content;
	e.name = 'notestype';
	var tareas = document.querySelectorAll('[name="notes"]');
	for (var i = 0; i < tareas.length; i++) {
		tareas[i].value = v;
	}
	var radios = document.querySelectorAll('[name="notestype"]');
	for (var i = 0; i < radios.length; i++) {
		if(radios[i].className == 'radio'){
			radios[i].checked = false;
		}
	}
};

function password_expiration(date_expiration,days_expiration){

  	var expirationDate=date_expiration;
	var today = new Date();
	...
	today_alert = addDays(myDate,days_expiration);
	today_alert=formatDate(today_alert);
	
	if (expirationDate>=today && expirationDate<=today_alert) {
		$('.expiration_notification').show();
	}else
	{
		$('.expiration_notification').hide();
	}
}
