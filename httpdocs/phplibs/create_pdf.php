...
require_once '../../../phplibs/fpdf17/fpdf.custom.php';
require_once '../../../phplibs/parsexml/src/autoload.php';
require_once '../../../phplibs/qrcode/qrcode.class.php';
require_once '../../../cfdi/common/php/letras.php';

function test($xml){
$parser = new cfdiMx\Parser($xml);
$xml = $parser->jsonSerialize();
	
}
function createPDF($id_orders,$xml_string='',$type="show"){
	if($xml_string == '')
		return '';
	// Parse XML
	...
	// Datos del Emisor
	$factura['empresa_nombre']		= $xml_factura_emisor["@atributos"]["nombre"];
	$factura['empresa_rfc']			= $xml_factura_emisor["@atributos"]["rfc"];
	...
	$factura['empresa_domiclio']	.= strtoupper("Mexico D.F. C.P. ").$xml_factura_emisor["DomicilioFiscal"]["@atributos"]["codigoPostal"];	
	...
	// Datos del Cliente
	$factura['cliente_nombre']		= $xml_factura_cliente["@atributos"]["nombre"];
	$factura['cliente_rfc']			= $xml_factura_cliente["@atributos"]["rfc"];
	...
	$factura['lugar_expedicion']	= $xml_factura_datgral["LugarExpedicion"];
	// Datos entrega
	...
	$factura['num_pedido']			= $id_orders;
	$factura['num_contrarecibo']	= "";
	$factura['dias_credito']		= "";
	$factura['condiciones']			= $xml_factura_datgral["condicionesDePago"];
	$factura['lote']				= "";
	$factura['forma_pago']			= $xml_factura_datgral["formaDePago"];
	$factura['num_cuenta']			= $xml_factura_datgral["NumCtaPago"];
	...
	// Importe con letras
	$letras = new EnLetras();
	$factura['monto_letra']	= utf8_decode($letras->ValorEnLetras($totales['total']));
	// Impuestos
...
function invoice_header($pdf, $factura){
	...
	$y = 15;
	// Logo
	$pdf->Image(ROOT_FOLDER.'/../../../../sitimages/default/inbound_innova.gif', 10, 15, 35, 13.7);
	// Datos de la empresa
	...
	$y += 4;
	$pdf->SetXY($x, $y);	
	$pdf->MultiCell(70, 3, utf8_decode($factura['empresa_domiclio']), 'L');
	$y += 6;
	$pdf->SetXY($x, $y);	
	$pdf->Cell(70, 2, '', 'L');
	$y += 3;
	...
	$pdf->SetXY($x, $y);
	$pdf->Cell(70, 5, utf8_decode('INFORMACIÓN DEL CLIENTE'), 'T');
	$pdf->Cell(60, 5, utf8_decode('INFORMACIÓN DE COMPRA'), 'T');	
	$y += 6;
	// Cliente
	$pdf->SetFont('Arial', '', 7);
	$pdf->SetXY($x+2, $y);
	...
	$pdf->SetFont('Arial', '', 7);
	$pdf->SetXY($x, $y);
	$pdf->Cell(65, 3.5, 'Lote: '.$factura['lote']);
	
	// Info. adicional
	$x = 10;
	$y = 61;	
	$pdf->SetXY($x, $y);
	$pdf->Cell(195, 4, utf8_decode('Expedido en: '.$factura['lugar_expedicion']), 'T', 0, 'R');	
	$y += 4;
}
