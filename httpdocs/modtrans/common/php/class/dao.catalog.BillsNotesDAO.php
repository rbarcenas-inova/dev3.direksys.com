...
require_once 'dto.catalog.BillsNotesDTO.php';
 ...
 */
class BillsNotesDAO extends BaseDAO implements InterfaceDAO {
    ...
    public function insertRecord(&$objectDTO) {
        ...
        if ($objectDTO instanceof BillsNotesDTO) {

            if ($this->connectDB()) {
                $cadena_sql = "INSERT INTO " . $objectDTO->getTableSource() . " (" . $objectDTO->getStringTableMetadata(TRUE) . ") VALUES ";
                $cadena_sql .="(
                    " . $this->cleanSqlValue($objectDTO->getID_bills()) . "," . $this->cleanSqlValue($objectDTO->getNotes()) . ",
