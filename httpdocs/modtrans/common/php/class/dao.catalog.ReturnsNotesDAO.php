...
require_once 'dto.catalog.ReturnsNotesDTO.php';
 ...
 */
class ReturnsNotesDAO extends BaseDAO implements InterfaceDAO {
    ...
    public function insertRecord(&$objectDTO) {
        ...
        if ($objectDTO instanceof ReturnsNotesDTO) {
            if ($this->connectDB()) {
                $cadena_sql = "INSERT INTO " . $objectDTO->getTableSource() . " (" . $objectDTO->getStringTableMetadata(TRUE) . ") VALUES ";
                $cadena_sql .= "(
                    " . $this->cleanSqlValue($objectDTO->getID_returns()) . "," . $this->cleanSqlValue($objectDTO->getNotes()) . ",
