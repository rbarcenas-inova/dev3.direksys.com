...
require_once 'dto.catalog.OrdersDateCodDTO.php';
 ...
 */
class OrdersDateCodDAO extends BaseDAO implements InterfaceDAO {
    ...
    public function insertRecord(&$objectDTO) {
        ...
        if ($objectDTO instanceof OrdersDateCodDTO) {

            if ($this->connectDB()) {
                $cadena_sql = "INSERT INTO " . $objectDTO->getTableSource() . " (" . $objectDTO->getStringTableMetadata(TRUE) . ") VALUES ";
                $cadena_sql .="(
                    " . $this->cleanSqlValue($objectDTO->getID_orders()) . "," . $this->cleanSqlValue($objectDTO->getID_warehouses()) . ",
                    " . $this->cleanSqlValue($objectDTO->getDateCod()) . "," . $this->cleanSqlValue($objectDTO->getDateCancelled()) . ",
