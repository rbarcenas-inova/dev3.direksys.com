...
require_once 'dto.catalog.OrdersNotesDTO.php';
 ...
 */
class OrdersNotesDAO extends BaseDAO implements InterfaceDAO {
    ...
    public function insertRecord(&$objectDTO) {
        ...
        if ($objectDTO instanceof OrdersNotesDTO) {

            if ($this->connectDB()) {
                $cadena_sql = "INSERT INTO " . $objectDTO->getTableSource() . " (" . $objectDTO->getStringTableMetadata(TRUE) . ") VALUES ";
                $cadena_sql .="(
                    " . $this->cleanSqlValue($objectDTO->getID_orders()) . "," . $this->cleanSqlValue($objectDTO->getNotes()) . ",
