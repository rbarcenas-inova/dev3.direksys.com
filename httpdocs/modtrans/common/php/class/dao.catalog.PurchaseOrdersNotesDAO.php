...
require_once 'dto.catalog.PurchaseOrdersNotesDTO.php';
 ...
 */
class PurchaseOrdersNotesDAO extends BaseDAO implements InterfaceDAO {
    ...
    public function insertRecord(&$objectDTO) {
        ...
        if ($objectDTO instanceof PurchaseOrdersNotesDTO) {

            if ($this->connectDB()) {
                $cadena_sql = "INSERT INTO " . $objectDTO->getTableSource() . " (" . $objectDTO->getStringTableMetadata(TRUE) . ") VALUES ";
                $cadena_sql .="(
                    " . $this->cleanSqlValue($objectDTO->getID_purchaseorders()) . "," . $this->cleanSqlValue($objectDTO->getNotes()) . ",
