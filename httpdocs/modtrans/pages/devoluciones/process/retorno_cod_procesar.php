...
require_once '../../../common/php/class/dto.catalog.OrdersDateCodDTO.php';
require_once '../../../common/php/class/dao.catalog.OrdersDateCodDAO.php';
require_once '../../../common/php/class/dto.catalog.AdminLogsDTO.php';
require_once '../../../common/php/class/dao.catalog.AdminLogsDAO.php';
require_once '../../../common/php/class/com.wms.WarehouseManager.php';

include_once '../../../common/php/functions/orders_functions.php';
include_once '../../../common/php/functions/auth_functions.php';
include_once '../../../common/php/functions/products_functions.php';
...
$ID_VIRTUAL_WAREHOUSE = 0;  //-- Chofer
$ID_WAREHOUSE = 0;  //-- almacen de retorno
//-- TIPOS DE DEVOLUCION
/*
...
$order_status_prd = "Undeliverable";
$order_status_pay = "None";
$order_ptype = "COD";
//$order_ptype = "Credit-Card";
...
//- Info del sistema
$id_admin_users = isset($usr['id_admin_users']) ? $usr['id_admin_users'] : 6;
...
//-- Informacion del warehouse virtual: chofer
$ID_VIRTUAL_WAREHOUSE = isset($_POST['lstVirtualWarehouse']) ? $_POST['lstVirtualWarehouse'] : $in['lstVirtualWarehouse'];
    ...
    if ($orderNotesDAO->insertRecord($orderNotesDTO)) {
        ...
        if ($exito_inserted_products) {
            ...
            $paymentAmount = (floatval($orderNet - $orderDisc) * (1+ $taxPercent)) + $orderShip;
             if (createOrderPayment($ID_orders,$paymentAmount)) { 
                ...
                if ($ordersDAO->updateOrderTotals($ordersDTO)) {
                    ...
                    */
                    //- procesa los datos del retorno del warehouse
                    createOrderDateCOD($ID_orders, $ID_VIRTUAL_WAREHOUSE);
                    $response_cod_return = processWarehouseCODReturn($_SESSION['ARRAY_ORDER_PARTS'], $ID_VIRTUAL_WAREHOUSE, $ID_orders);

                    if (!empty($response_cod_return['falied_upc'])) {
                        $process_msg .= "<br>La orden ha sido cancelada, pero el proceso de retorno del inventario fallo debido a los siguientes UPCs: " . implode(', ', $response_cod_return['falied_upc'] . "<br>");
                        foreach ($response_cod_return['arr_response'] as $response) {
                            $process_msg .= $response["msgProcessErrors"] . "<br>";
                        }
                    }
                    if (!$response_cod_return['exito']) {
                        $exito = 0;
                        $process_msg .= "<br>Ocurrieron los siguientes errores al procesar el retorno de inventario: <br>";

                        foreach ($response_cod_return['arr_response'] as $response) {
                            $process_msg .= $response["msgErrors"] . "<br>";
                        }                        
                    }else{
                        $exito = 1;
                    }                    
...
function createOrderPayment($ID_orders, $amount = 0) {
    $payment_type = 'COD';
...
//-- Para Warehouse Virtual
function createOrderDateCOD($ID_orders, $ID_warehouses) {
    global $dateCOD;
    global $id_admin_users;
    ...
    $orderDateCodDTO = new OrdersDateCodDTO();
    $orderDateCodDAO = new OrdersDateCodDAO();

    $orderDateCodDTO->setID_orders($ID_orders);
    $orderDateCodDTO->setID_warehouses($ID_warehouses);
    $orderDateCodDTO->setDateCod($dateCOD);
    $orderDateCodDTO->setDateCancelled(date('Y-m-d'));
    $orderDateCodDTO->setStatus($status);
    $orderDateCodDTO->setID_admin_users($id_admin_users);

    if ($orderDateCodDAO->insertRecord($orderDateCodDTO)) {
        $exito = TRUE;
    }
    return $exito;
}

function processWarehouseCODReturn($array_order_parts, $from_wh, $id_orders) {
    global $id_admin_users;
    ...
    $whManager->setID_admin_users($id_admin_users);
    ...
    foreach ($array_order_parts as $array_part_info) {
        ...
        $part_tax = $array_part_info['part_tax'];
        $part_total = $array_part_info['part_total'];
        $to_wh = $array_part_info['id_warehouse_dest'];

        $id_sku_products = 400000000 + (int) $ID_part;

        //-- obtiene los datos del UPC
        $skusDTO = new SkusDTO();
        $skusDAO = new SkusDAO();

        $skusDTO->setID_sku_products($id_sku_products);
        $arr_result = $skusDAO->selectRecords($skusDAO);

        if (!empty($arr_result)) {
            $skusDTO = $arr_result[0];
        }

        if ($skusDTO->getIsSet() != 'Y') {
            $transfer_whresponse = $whManager->transfer_warehouse($from_wh, $to_wh, $id_sku_products, $part_qty, $id_orders);
            $arr_response[] = $transfer_whresponse;

            if ($transfer_whresponse['successTransfer'] == FALSE) {
                $failed_upc[] = $skusDTO->getUPC();
            }

            if ($transfer_whresponse['successProcess']) {
                $exito = TRUE;
            } else {
                $exito = FALSE;
                break;
    ...
    if (empty($failed_upc)) {
        ...
        $message = "The Order has been cancelled, however returning inventory  process failed beacuse of the upcs: " . implode(', ', $failed_upc);
    }
    ...
    if ($exito) {
        ...
        $orderNotesDTO->setID_orders($id_orders);
        $orderNotesDTO->setNotes($message);
        $orderNotesDTO->setType("High");
        $orderNotesDTO->setID_admin_users($id_admin_users);
        $orderNotesDAO->insertRecord($orderNotesDTO);

        $dbh = new DbHandler();

        $dbh->connect();
        $dbh->executeSQLcommand("UPDATE sl_orders_datecod SET Status='Inactive', DateCancelled=CURDATE() WHERE ID_orders='$id_orders' AND Status='Active';");
        $dbh->executeSQLcommand("UPDATE sl_orders_products SET Cost='0' WHERE ID_orders = '$id_orders';");
        $dbh->executeSQLcommand("UPDATE sl_warehouses_batches_orders INNER JOIN sl_orders_products USING(ID_orders_products) SET sl_warehouses_batches_orders.Status = 'Returned' WHERE ID_orders = '$id_orders';");
        $dbh->executeSQLcommand("INSERT INTO sl_orders_notes SET ID_orders='$id_orders', Notes='Order Cancelled \n COD failed to deliver the merchandise\n\n', Type='High',Date=Curdate(),Time=CURTIME(),ID_admin_users='$id_admin_users';");
        $dbh->disconnect();
    }

    $response["exito"] = $exito;
    $response["arr_response"] = $arr_response;
    $response["falied_upc"] = $failed_upc;
