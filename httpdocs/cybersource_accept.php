...
/*if (isset($_REQUEST['RequestID']) and $_REQUEST['RequestID']) {*/
	...
	/*$myXMLData = trim(' <?xml version="1.0" encoding="UTF-8"?> <!DOCTYPE CaseManagementOrderStatus SYSTEM "https://ebctest.cybersource.com/ebctest/reports/dtd/cmorderstatus_1_1.dtd"> <CaseManagementOrderStatus xmlns="http://reports.cybersource.com/reports/cmos/1.0" MerchantID="banorteixe_inova1" Name="Case Management Order Status" Date="2017-10-26 17:15:19" Version="1.1"> <Update MerchantReferenceNumber="5313279" RequestID="5137282160176291503008"> <OriginalDecision>REVIEW</OriginalDecision>  <NewDecision>ACCEPT</NewDecision> <Reviewer>banorteixe_inova1</Reviewer> <ReviewerComments>This is a test message(1).</ReviewerComments> <Queue>TestQueue(1)</Queue> <Profile>TestProfile(1)</Profile> </Update> </CaseManagementOrderStatus>');*/
	...
	if ($row['ID_orders'] > 0) {
		...
		if ($NewDecision == 'ACCEPT') {
			...
			if ($row2['Status'] == 'Approved') {
				...
				$conn->query($sql);
				echo "<br>sql=".$sql;*/
				$arr_resp_pay = explode("\n", $res_payw);

				$url_accounting_ac = $cfg['url_payment_acounting_ac']."?e=".$e."&id_orders=".$row['ID_orders']."&id_orders_payments=".$row['ID_orders_payments'];
				$res_accounting_ac = "";
				if (trim($arr_resp_pay[0]) == 'OK') {
					$res_accounting_ac = file_get_contents($url_accounting_ac);
				}
				echo "<br>res_accounting_ac=".$res_accounting_ac;
				$res_accounting_ac = $conn->real_escape_string($res_accounting_ac);
				$sql = "INSERT INTO sl_debug (cmd, id, log, Date, Time, ID_admin_users) VALUES ('cybersource_res_acc_ac', $row[ID_orders], '".$res_accounting_ac."', CURDATE(), CURTIME(), 6);";
				$conn->query($sql);
				echo "<br>sql=".$sql;
			} else {
