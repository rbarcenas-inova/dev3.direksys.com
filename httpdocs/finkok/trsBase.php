...
function save_auth_data($sid, $sdata) {
    ...
    if ($cfg['gensessiontype'] == 'mysql') {
        $sth = mysqli_query($conn, "INSERT INTO admin_sessions SET ses='$sid', Content='" . filter_values($output) . "', CreatedBy=$sdata[id_admin_users], CreatedDateTime=NOW(),ExpDateTime=NOW();");
    } else {
...
function load_cfg($tbl_name) {
    ...
    $db_cols = array();
    $sth = mysqli_query($conn, "describe $tbl_name;");
