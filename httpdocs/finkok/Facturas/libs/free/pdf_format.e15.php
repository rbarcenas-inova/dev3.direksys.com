...
function pdf($id_invoices,$xml_string='',$type="show",$invoiceInfo = array(), $tt){
	...
	$datosEnvio = $conn->query("Select `id_orders`, `customer_shpaddress_code`,`customer_shpaddress_alias`,`customer_shpaddress_contact`,`customer_shpaddress_street`,`customer_shpaddress_num`,`customer_shpaddress_num2`,`customer_shpaddress_urbanization`,`customer_shpaddress_district`,`customer_shpaddress_city`,`customer_shpaddress_state`,`customer_shpaddress_country`,`customer_shpaddress_zipcode` from cu_invoices inner join cu_invoices_lines using(ID_invoices) where id_invoices = $id_invoices group by ID_invoices limit 1")->fetch(PDO::FETCH_OBJ);
	$notas = '';
 ...
 *
 */
function invoice_header($pdf, $factura, $tt){
	$x = 10;
	$y = 10;
	...
	// Logo
	if($tt == 1)
		$pdf->Image(ROOT_FOLDER.'/../../sitimages/default/e14.png', 10, 15, 35, 13.7);
	else
		$pdf->Image(ROOT_FOLDER.'/../../sitimages/default/inbound-kea.png', 10, 15, 35, 13.7);
	// Datos de la empresa
	...
	// $pdf->SetXY($x+2, $y);
	// $pdf->SetXY($x+2, $y);
	// $pdf->MultiCell(60, 3.5, utf8_decode(strtoupper($factura['datos_entrega'])));
	// $y += 10;
