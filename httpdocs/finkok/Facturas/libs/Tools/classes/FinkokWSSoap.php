...
class FinkokWSSoap{
	...
	public static function Timbrar($xmlString,$rfc,$invoice=0){
		...
		if(!isset($response->stampResult->UUID)){
			if(is_array($response->stampResult->Incidencias)){
				...
				foreach ($inc as $key => $value) {
					...
					$errores = htmlentities($errores, ENT_QUOTES);
				}
			}else{
		...
		if(strlen($response->stampResult->xml) > 0){
			MysqlBD::executeQuery("update cu_invoices set `Status`='Certified', xml_uuid='{$response->stampResult->UUID}' where id_invoices='$invoice'");
			$save = array(
				'ID_invoices'=>$invoice,
