...
class Estandar{
	...
	public static function getAddendaStruct($invoice){
		...
		$data = array(
			...
			'cfdi:Receptor'=>array(
				...
				'cfdi:Domicilio' => array(
					...
					'@municipio' => limpiar($invoice->customer_address_city) == '' ? limpiar($invoice->customer_address_urbanization) : limpiar($invoice->customer_address_city),
					...
					'@localidad' => limpiar($invoice->customer_address_city) == '' ? limpiar($invoice->customer_address_urbanization) : limpiar($invoice->customer_address_city)
					)
				)
			);
		$metodosPago = array(
		...
		for ($i=0; $i < $conceptos->num(); $i++) {
	        ...
	        $customer_upc = $conn->query("select sku_customers from sl_customers_parts
			where 
			id_parts = ".substr($id_product, -4)." and id_customers = $invoice->ID_customers")->fetch(\PDO::FETCH_OBJ)->sku_customers;
	        if($customer_upc != ''){
	        	$concepto->ID_sku = $customer_upc;
	        	$concepto->UPC = $customer_upc;
	        }
