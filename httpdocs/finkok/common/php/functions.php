	...
	if ($va['matches']>0) {
		...
		while ($rec = mysql_fetch_assoc($sth1)) {
			...
			if($ptype != 6 and $ptype != 4){ 
					...
					$name .=load_name('sl_noninventory','ID_noninventory',substr($rec['ID_products'],5,4),'Name');
					
					if(load_name('sl_noninventory','ID_noninventory',substr($rec['ID_products'],5,4),'ServiceType') == 'Refill'){
...
// Last Time Modified by RB on 12/13/2010: Se agrega validacion de $cses['items_'.$i.'_fpprice']
	...
	for ($i=1;$i<=$cses['items_in_basket'];$i++)
	{
	    ...
	    //Calcular Shp,Disc,Tax, etc. aqui
			...
			$id_noninventory = 0;		
			$id_noninventory = intval(load_name('sl_products','ID_products',substr($cses['items_'.$i.'_id'],-6),'ID_noninventory_related'));
			if($id_noninventory > 0){
					## Producto tiene servicio relacionado
					$insert2='';
					$price = 0;
					$price = &load_name ('sl_noninventory','ID_noninventory',$id_noninventory,'SPrice');
					$id_noninventory += 600000000;
					
					$insert2 = "INSERT INTO sl_orders_products set ID_products='$id_noninventory',ID_orders='".mysql_real_escape_string(intval($cses['id_orders']))."',Quantity=1,Related_ID_products='".mysql_real_escape_string($cses['items_'.$i.'_id'])."',SalePrice='".$price."',Shipping=0,Cost=0,Tax=0,Discount=0,FP=1,Status='Active',Date=CURDATE(),Time=CURTIME(),ID_admin_users='".$cfg['id_admin_users']."'";
...
# --------------------------------------------------------
	...
	if ($choices >	1){
		...
		$output = "<br><a href='#$module' class='l_text'  id='ajax_btn' name='choices_btn' onClick=\"popup_show('ajax_windows', 'ajax_drag', 'ajax_exit', 'element-right', -1, -1,'$module');loadDoc('/cgi-bin/nsc_admin/common/apps/ajaxbuild.cgi?ajaxbuild=choices&id_products=$family&url=$url&$param');\">$content_link</a> ";
	}
	return $output;
}	
...
//					my ($sth) = &Do_SQL("SELECT * FROM sl_noninventory WHERE ID_noninventory ='".substr($cses['servis_'.$i.'_id'],5,4)."'");
...
//					$linkserv="<a href='$script_url?cmd=console_order&step=$in{'step'}&dropser=$i&id_noninventory=$rec->{'ID_noninventory'}#tabs'><img src='$va{'imgurl'}/$usr{'pref_style'}/b_drop.png' title='Drop' alt='' border='0'></a>";
//				}else{
//					$linkserv='';
//				}
//				if($rec->{'SalesPrice'}=='Fixed'){
...
//							<a href="/cgi-bin/cc-inbound/console.cgi?cmd=console_order&step=2&action=search&id_products=|.substr($cses['servis_'.$i.'_id'],5).qq|"><img src='[va_imgurl]/[ur_pref_style]/b_view.gif' title='View' alt='' border='0'></a>
//							$serviceid</td>
...
# Last Modified RB: 10/09/09  16:14:14 -- Se valida tipo de pago para promos
	...
	#		$cadtypespay.=$cses['items_'.$i.'_paytype'];
			...
			$prelated = &load_name('sl_products_prior','ID_products',substr($cses['items_'.$i.'_'.$tprod],3,6),'ID_noninventory_related');
			if(strlen($prelated) < 4)
				$prelated = &load_name('sl_products','ID_products',substr($cses['items_'.$i.'_'.$tprod],3,6),'ID_noninventory_related');
			if(strlen($prelated) == 4){
				$stype = &load_name('sl_noninventory','ID_noninventory',$prelated,'ServiceType');
			}
...
#		}
		...
		if($cses['paytype']=="Layaway")
		{
			...
			{
				$pricetoapply= load_name('sl_noninventory','ID_noninventory',$cfg['membershipservid'],'SPrice');
