...
class CurlClient{
	...
	*/
	public static function request($config = array()){
		...
		curl_setopt($curl, CURLOPT_CONNECTTIMEOUT, 4500); 
		curl_setopt($curl, CURLOPT_TIMEOUT,        4500); 
		curl_setopt($curl, CURLOPT_RETURNTRANSFER, TRUE );
		$response = curl_exec($curl);
		self::$errors = self::curlError(curl_error($curl));
		self::$request = curl_getinfo($curl, CURLINFO_HEADER_OUT);
		// print_r($params);
		curl_close($curl);
		if(isset($parseFrom)){
			if($parseFrom == 'json')
	...
	*/
	...
	private static function curlError($index){
		$error_codes=array(
