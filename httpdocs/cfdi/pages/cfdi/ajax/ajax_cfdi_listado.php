...
if ($criteria_change == "1") {
    ...
    $_SESSION['TOTAL_RESULTADOS_CFDI'] = $total_results;
...
/*if(isset($_SESSION['s_total']) and  $total_results != ''){
    $s_total = $_SESSION['s_total'];
}else{
    $s_total = $total_results;
    $_SESSION['s_total'] = $s_total;
}*/

$s_total = $_SESSION['TOTAL_RESULTADOS_CFDI'];
$s_from = $page == 1 ? 1 : ($results_per_page * $page)+1;
$s_to = $s_from + $results_per_page - 1;
$echo_string = generateHtml($vector_invoices,$s_from, $s_to, $s_total);
...
function generateHtml($vector_invoices,$s_from, $s_to, $s_total) {
    global $COMMON_PATH;
    global $file_path;
    global $e;
    global $page;
    ...
    if (!empty($vector_invoices)) {
        ...
        $str_html = "<h4>Mostrando : $s_from a $s_to de $s_total</h4>".'
        <table width="99%" align="center"  class="List">
                    <tr class="tableListColumn">
                        ...
                        #.'<td align="center" colspan="1"><img src="' . $COMMON_PATH . '/common/img/print.png" height="16px" width="16px" style="cursor: pointer" onclick="PrintPDF(ToPrint.value,2)" alt="Imprimir Seleccionados" /></td>'
                ...
                if($invoicesDTO->getStatus()!='Transition'){ // Omitir Status especificos
                    ...
                    if($invoicesDTO->getViewed()<1 && $invoicesDTO->getDocSerial()!=''){
                        ...
                        */
                        ...
                        $str_html .= '<a href="cfdi_doc.php?f='.base64_encode($file_pdf).'&id='.$invoicesDTO->getID_invoices().'&m=2" target="_blank" title="PDF EDX"><img src="' . $COMMON_PATH . '/common/img/pdf.gif" height="16px" width="16px" style="cursor: pointer"  /></a>&nbsp;';
                        $str_html .= '<a href="cfdi_doc.php?f='.base64_encode($file_pdf).'&id='.$invoicesDTO->getID_invoices().'&m=2&readXml=1" title="PDF Direksys" target="_blank"><img src="' . $COMMON_PATH . '/common/img/pdf.gif" height="16px" width="16px" style="cursor: pointer" /></a>&nbsp;';
                        $str_html .= '<a href="cfdi_doc.php?f='.base64_encode($file_xml).'&id='.$invoicesDTO->getID_invoices().'&m=2" target="_blank" ><img src="' . $COMMON_PATH . '/common/img/xml.gif" height="16px" width="16px" style="cursor: pointer" alt="XML" /></a>&nbsp;';
                        
                        $str_html .= '</td>';
                        $str_html .= '<td align="center">';
                        $str_html .= '<a href="cfdi_doc.php?f='.base64_encode($file_pdf).'&id='.$invoicesDTO->getID_invoices().'&m=0" target="_blank" alt="Descargar PDF"><img src="' . $COMMON_PATH . '/common/img/pdf_download.gif" height="16px" width="16px" style="cursor: pointer" /></a>&nbsp;';                
                        $str_html .= '<a href="cfdi_doc.php?f='.base64_encode($file_xml).'&id='.$invoicesDTO->getID_invoices().'&m=0" target="_blank" ><img src="' . $COMMON_PATH . '/common/img/xml_download.gif" height="16px" width="16px" style="cursor: pointer" alt="XML" /></a>&nbsp;';
                        $str_html .= '</td>';
