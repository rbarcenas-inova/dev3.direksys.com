...
app.get('/', function(req, res) {
  ...
  if(param.length > 0){
      ...
      "$(\"#tipo\" ).change(function() { " +
        "$(\"select option:selected\").each(function() { " +
            "if($( this ).text() == 'Informativo'){ \n" + 
              ...
              "subtipos.append(\"<option value='Informacion de Entrega'  >Informacion de Entrega  </option>\");\n" +
            "} \n" +
    ...
    html_tipificar = html_tipificar + "\n<center><h2>Nueva llamada de "+cliente[1]+"</h2></center>\n<center><h3>Tipificar la llamada</h3>\n<form id='tipificacion' name=\"tipificacion\" >\n<label>Area&nbsp;</label><select name=\"area\" id=\"area\"><option value=\"Telemarketing\" >Telemarketing</option><option value=\"Fidelizacion\" >Fidelizacion</option><option value=\"Internet\" >Internet</option></select>" + 
      "<br/><label>Tipo&nbsp;</label><select name=\"tipo\" id=\"tipo\"><option value=\"\" >-- seleccionar --</option><option value=\"Informativo\" >Informativo</option><option value=\"Tramites\" >Tramites</option><option value=\"Tickets\" >Tickets</option><option value=\"Supervision\" >Supervision</option><option value=\"Transferencia\" >Transferencia</option><option value=\"Confirmado con error\" >Confirmado con error</option><option value=\"Cancelado\" >Cancelado</option><option value=\"No confirmado\" >No confirmado</option><option value=\"Confirmado\" >Confirmado</option></select><br/>\n" + 
    ...
    var html_cola = "<center><input type=\"button\" href=\"/cgi-bin/mod/crm/dbman?cmd=opr_crmtickets&add=1&caller_id=" + cliente[1] + "\" " +
                      "onclick=\"window.location.href=this.getAttribute('href');return false;\" value=\"Crear un nuevo ticket\" class=\"button\"></center>";
    //</form>";
  //-------------------
  // TMK
  //-------------------
    var ordenes = MySQL.query("SELECT cus.ID_customers, cus.FirstName, cus.LastName1, cus.LastName2, cus.Phone1, cus.Phone2, cus.Cellphone, ord.ID_orders, date_format(ord.Date,'%d/%m%Y') Date, ord.Status " +
      ...
      , function(rst_tickets){
        ...
        rst_tickets.forEach(function(ticket) {
          ...
          html_tickets = html_tickets + "<tr style=\"background-color:#e3e8e8;cursor:pointer;\" href=\"dbman?cmd=opr_crmtickets&amp;add=1&amp;caller_id="+cliente[1]+"&amp;id_ref="+ticket.ID_orders+"\" onclick=\";window.location.href=this.getAttribute('href');return false;\">\n";
          html_tickets = html_tickets + "<td>"+ticket.ID_crmtickets+"</td><td>"+ticket.ID_ref+"</td><td>"+ticket.Type+"</td><td>"+ticket.Description+"</td><td>"+color+"</td></tr>\n";
        });
